...
Each language also gets its own full-content file, such as `/fr/llms-full.txt`.
Your default language has one too (for example `/en/llms-full.txt`). The root
`llms-full.txt` still contains every language.
