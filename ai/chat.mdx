...
<Tabs>
  <Tab title="API onboarding">
    A developer integrating your API asks: *"How do I authenticate requests?"*

    Chat retrieves your authentication page, returns the exact steps with a code example, and links to the source page.
  </Tab>
  <Tab title="Troubleshooting">
    A user stuck on a problem asks: *"My build is failing with a timeout error"*

    Chat finds your troubleshooting guide and returns the relevant fix without the user having to scan multiple pages.
  </Tab>
  <Tab title="Quick reference">
    A returning user asks: *"What parameters does the /users endpoint accept?"*

    Chat pulls the endpoint documentation and returns the parameter table with types and descriptions.
  </Tab>
</Tabs>
