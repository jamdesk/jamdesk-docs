...
title: Codex
description: >-
  Codex is OpenAI's cloud coding agent for asynchronous, multi-file
  documentation batch tasks directly against GitHub repos.
---
...
Replace `your-project` with your Jamdesk subdomain — or, if you have a custom domain live, use it instead: `url = "https://docs.acme.com/_mcp"`. See [MCP Server](/ai/mcp-server) for endpoint details.
...
Multi-file restructures work especially well in this mode. Copy this prompt into Codex for a task with clear review criteria:

<Prompt title="Restructure a large README">
Restructure the 500-line `README.md` into focused Jamdesk documentation pages.

Acceptance criteria:

- Preserve every unique technical detail, command, warning, and code example from the README.
- Group the content into task-focused `.mdx` pages with descriptive filenames.
- Give every page `title` and `description` frontmatter, an opening paragraph, and What's Next cards.
- Add every new page to the appropriate group in `docs.json` and keep the page order logical.
- Use only Jamdesk components already listed in `components/overview.mdx`; do not invent components.
- Fix internal links for the new file structure and report any source link you cannot resolve.
- Do not delete the original README until all content is represented and the new pages pass the project's validation and broken-link checks.
</Prompt>
