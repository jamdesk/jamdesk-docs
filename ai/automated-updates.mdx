...
4. **Writes documentation** - Creates or updates pages following Jamdesk standards, focusing on explaining *why* (not just *what*), with simple examples first. Always review the output for AI-sounding filler — Claude writes better with a CLAUDE.md that sets the voice (see [Claude Code](/ai/claude-code))
