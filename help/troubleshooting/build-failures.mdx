...
description: Diagnose build failures with a checklist of common causes and fixes. Follow the steps here and use the related articles for deeper setup or troubleshooting.
---

Diagnose build failures with a checklist of common causes and fixes. Use it to identify configuration issues, missing files, and platform errors before escalating. Follow the checks in order to isolate the issue, then use the fixes or contact support if the problem persists. Screens in the dashboard may vary slightly by plan, but the steps are the same.
