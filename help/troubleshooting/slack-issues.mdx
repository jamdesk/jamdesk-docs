---
title: Slack Issues
description: Troubleshoot Slack integration issues, from missing notifications to authorization errors. Follow the steps here and use the related articles for deeper setup.
---

Troubleshoot Slack integration issues, from missing notifications to authorization errors. Use this guide to verify settings, test the connection, and reconnect the integration if needed. Follow the checks in order to isolate the issue, then use the fixes or contact support if the problem persists. Screens in the dashboard may vary slightly by plan, but the steps are the same.
