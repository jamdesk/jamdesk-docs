...
description: Troubleshoot billing problems such as failed payments or missing invoices. Follow the steps here and use the related articles for deeper setup or.
---

Troubleshoot billing problems such as failed payments or missing invoices. This guide walks through common fixes, what to check in the billing portal, and when to contact support. Follow the checks in order to isolate the issue, then use the fixes or contact support if the problem persists. Screens in the dashboard may vary slightly by plan, but the steps are the same.
