...
description: Fix common DNS issues when setting up custom domains. Covers propagation delays, CNAME conflicts, and verification failures.
---

DNS configuration issues are the most common cause of custom domain problems. This guide covers diagnosis and fixes.
...
**Symptom**: Can't add CNAME for `yourdomain.com` (without subdomain).

**Cause**: DNS standards prohibit CNAME records at the zone apex (root domain).

**Fix**: Use `docs.yourdomain.com` instead, or use Jamdesk's subpath hosting at `yourdomain.com/docs` with a reverse proxy.
...
- Disable "Always Use HTTPS" for docs subdomain
