...
<AccordionGroup>
  <Accordion title="How do I invite a team member?">
    Open your project, go to **Settings** → **Members**, type the person's email in the **Invite Member** field, and click **Invite**. They'll get an email with a link to join the project.
    ...
    Invited users need a Jamdesk account; if they don't have one, they're prompted to create it when accepting. Already a member, but the invite link says it's "already been accepted"? You don't need a new invitation. Just sign in and you'll land back in the project. See [Team Members](/help/projects/team-members) for roles and member limits.
  </Accordion>

  <Accordion title="How many team members can I have?">
    Every plan includes unlimited team members per project, at no extra charge.
  </Accordion>
  ...
  <Accordion title="How do I transfer project ownership?">
    Go to **Settings** → **Members**, then click **Transfer Ownership** next to the member you want to promote. The new owner must be an existing team member.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="How do I fix broken links automatically?">
    Run `jamdesk fix`. It repairs broken internal links that have an unambiguous correct target (typo'd anchors and cross-locale anchor drift), editing your local `.mdx` files after a preview. Use `jamdesk fix --dry-run` to see the planned changes first; only links whose corrected anchor is a real heading are touched, leaving ambiguous cases for manual review. See [Auto-fix broken links](/cli/fix-broken-links).
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Can I have public and private docs in the same site?">
    Yes. Mark internal pages with `private: true` in frontmatter, or list their paths under `auth.password.private[]` in `docs.json`. Public pages stay public; private ones show an unlock screen. Everything stays in a single project, so public and private pages share the same repo and build. See [Access Control](/setup/access-control) and [Password Protection](/setup/password-protection).
  </Accordion>

  <Accordion title="How do I lock my entire docs site behind a password?">
    Set `auth.password.enabled: true` in `docs.json`, push the change, then set a password in **Project Settings → Password Protection** in the dashboard. Every visitor sees an unlock screen until they enter it. See [Password Protection](/setup/password-protection).
  </Accordion>

  <Accordion title="Can readers log in with their own account instead of a shared password?">
    Yes, on Enterprise plans. Single Sign-On lets readers authenticate against your identity provider (Okta, Google Workspace, Azure AD, etc.) with no shared passwords, a full audit trail, and instant deprovisioning. See [SSO](/setup/sso) or [contact Enterprise sales](https://jamdesk.com/pricing).
  </Accordion>

  <Accordion title="Should I split internal and external docs into two projects?">
    Usually no. A single project with `private: true` on the internal pages is simpler because you keep everything in one repo and manage it from one dashboard. Use two projects only when the audiences need fully separate branding, custom domains, or analytics. See [Access Control](/setup/access-control) for the patterns.
  </Accordion>

  <Accordion title="What's the difference between an editor and a reader?">
    **Editors** write the docs by committing MDX to your connected GitHub repo, so anyone with push access to your docs branch can publish a change. **Readers** view the published site: everyone (public), anyone with the password (password mode), or anyone signed in via your IdP (SSO). **Dashboard team members** are a separate concept: they manage builds and analytics but don't author content directly. See [Access Control](/setup/access-control).
  </Accordion>
</AccordionGroup>
