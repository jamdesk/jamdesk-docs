...
<AccordionGroup>
  ...
  <Accordion title="Can readers log in with their own account instead of a shared password?">
    Yes, on Enterprise plans. Single Sign-On lets readers authenticate against your identity provider (Okta, Google Workspace, Azure AD, etc.) — no shared passwords, full audit trail, instant deprovisioning. See [SSO](/setup/sso) or [contact Enterprise sales](https://www.jamdesk.com/pricing).
  </Accordion>
  ...
  <Accordion title="What's the difference between an editor and a reader?">
    **Editors** write the docs — docs are edited by committing MDX to your connected GitHub repo, so anyone with push access to your docs branch can publish a change. **Readers** view the published site — anyone (public), anyone with the password (password mode), or anyone signed in via your IdP (SSO). **Dashboard team members** are a separate concept: they manage builds and analytics but don't author content directly. See [Access Control](/setup/access-control).
  </Accordion>
</AccordionGroup>
