...
<AccordionGroup>
  ...
  <Accordion title="What's the difference between Jamdesk and other docs platforms?">
    Jamdesk is built around MDX and GitHub — your docs are plain text files in a repo, not locked in a proprietary editor. Push to GitHub and your site deploys in under 60 seconds with no build configuration or CI/CD pipeline. Every site also gets `llms.txt` and an MCP server automatically, so AI tools like Claude and ChatGPT can access your current docs directly. Every site also includes a built-in [AI chat assistant](/ai/chat) that answers visitor questions directly from your docs.
  </Accordion>
</AccordionGroup>
