...
<AccordionGroup>
  <Accordion title="Can I have public and private docs in the same site?">
    Yes. Mark internal pages with `private: true` in frontmatter, or list their paths under `auth.password.private[]` in `docs.json`. Public pages stay public; private ones show an unlock screen. One project, one repo, one build. See [Access Control](/setup/access-control) and [Password Protection](/setup/password-protection).
  </Accordion>
  ...
  <Accordion title="Should I split internal and external docs into two projects?">
    Usually no. A single project with `private: true` on the internal pages is simpler: same repo, same build, one dashboard. Use two projects only when the audiences need fully separate branding, custom domains, or analytics. See [Access Control](/setup/access-control) for the patterns.
  </Accordion>
