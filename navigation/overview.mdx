...
Every named group is collapsible, whether it appears directly under a tab or inside another group. Groups start closed unless they contain the current page or set `expanded: true`. On initial load and route changes, the current page's full ancestor chain opens automatically so the active link is revealed; visitors may still collapse the active group manually.

| Group type | Default behavior |
| --- | --- |
| **Named group** | Collapsed until active, manually opened, or configured with `expanded: true`. Clicking a closed group opens it and navigates to its first page. Clicking an open group closes it. |
...
<Frame caption="A named group in its collapsed state. The chevron points right and the child pages are hidden.">
...
| `expanded` | boolean | No | Open the group by default on first page load. Works for top-level and nested named groups. The current page's ancestor groups open automatically on initial load and route changes. |
