---
title: Embed a Page
description: Add a "What's new?" button to your own app that opens your Jamdesk changelog in a modal, with an unread dot. Requires a single script tag and no build step.
---
...
Open your dashboard, go to **Integrations → What's New widget**, set the page and launcher options, and copy the generated snippet. It looks like this:
...
The widget is open source, and the hosted snippet above always serves the latest version, which is the right default for most sites. When you'd rather freeze a known version or serve the file yourself, the [`jamdesk-widget`](https://github.com/jamdesk/jamdesk-widget) repository offers two more ways to load it.
...
`data-page` opens any path on your docs site, not only `/changelog`. The modal renders whatever page you name, such as a single announcement or a migration note, stripped of its site chrome. Point it wherever a focused, in-context page helps:
