...
description: Liez les pages, ancrez vers des titres spécifiques et référencez des URL externes. Inclut la navigation par Card et la vérification des liens brisés.
---

MDX prend en charge les liens Markdown standard ainsi que des modèles spécifiques à Jamdesk pour créer des liens entre les pages, ancrer vers des titres et référencer des URL externes.
...
Créez un lien vers des sites externes :
...
<AccordionGroup>
  ...
  <Accordion title="Créez un lien vers le bon niveau" icon="sitemap">
    Créez un lien vers la section pertinente la plus spécifique :
  ...
  <Accordion title="Maintenez les liens à jour" icon="rotate">
    Les liens brisés frustrent les lecteurs. Exécutez régulièrement des vérifications de liens :
...
Le CLI Jamdesk peut analyser les liens brisés :
...
Exécutez cette commande avant de déployer pour repérer les fautes de frappe et les références obsolètes.

### Correction automatique des liens brisés

Lorsqu'un lien brisé a une cible correcte non ambiguë (une ancre mal orthographiée, ou une dérive d'ancre inter-locale où une page traduite a renommé ses titres), `jamdesk fix` le corrige automatiquement pour vous :
...
Il ne réécrit un lien que lorsque l'ancre corrigée est un titre réel sur la page cible, laissant tout ce qui est ambigu pour une révision manuelle. Voir [Correction automatique des liens brisés](/fr/cli/fix-broken-links).
...
Pour les sites utilisant `hostAtDocs` (documentation hébergée sur un sous-chemin comme `/docs`), les liens internes sont **automatiquement préfixés** au moment de l'exécution. Écrivez des chemins relatifs à la racine de votre documentation — le préfixe `/docs` est ajouté pour vous :
...
<Note>
  ...
  Tout lien `/path` dans le contenu de votre documentation est considéré comme un lien interne vers la documentation.
</Note>

## Renommer des pages

Lorsque vous renommez une page, les liens sont rompus. Utilisez le CLI pour mettre à jour toutes les références :

```bash
jamdesk rename api/old-name.mdx api/new-name.mdx
```

Cela effectue automatiquement les actions suivantes :
- Renomme le fichier
- Met à jour la navigation `docs.json`
