---
title: Vue d'ensemble du CLI
description: >-
  Prévisualisez la documentation en local, validez la configuration, vérifiez
  les liens rompus et migrez vos plateformes avec le CLI open source de
  Jamdesk.
---

Le CLI Jamdesk vous permet de prévisualiser la documentation en local, de valider la configuration, de vérifier les liens rompus et de migrer depuis d'autres plateformes. Il est open source sous licence [Apache License 2.0](https://github.com/jamdesk/jamdesk-cli).
...
<Accordion title="jamdesk dev" icon="play" defaultOpen>
  ...
  **Fonctionnalités :**
  - Validation automatique au démarrage (schéma docs.json, syntaxe MDX et spécifications OpenAPI référencées ; une spécification invalide arrête le serveur afin que vous la détectiez avant de déployer)
  ...
  **Options :**
  ...
  | Flag | Description |
  |------|-------------|
  | `-p, --port <port>` | Port d'exécution (par défaut : 3000) |
...
<Accordion title="jamdesk init" icon="folder-plus">
  ...
  - Fichier de configuration `docs.json`
  - Exemples de pages MDX
...
<Accordion title="jamdesk login" icon="right-to-bracket">
  ...
  <Card title="Guide d'authentification" icon="key" href="/fr/cli/authentication">
    Flux d'authentification basé sur le navigateur, gestion des sessions et dépannage
  </Card>
</Accordion>
...
<Accordion title="jamdesk validate" icon="check">
  Valide la configuration de votre `docs.json`, la syntaxe MDX et les spécifications OpenAPI.
  ...
  **Vérifie :**
  ...
  - Erreurs de syntaxe MDX (par ex. caractères `<` non échappés)
  ...
  **Options :**
  ...
  | Flag | Description |
  |------|-------------|
  | `--skip-mdx` | Ignore la validation de la syntaxe MDX |
  | `-v, --verbose` | Affiche une sortie de validation détaillée |
...
<Accordion title="jamdesk fix" icon="wrench">
  ...
  - **Ancres mal orthographiées** : un fragment comme `#instalation` qui devrait clairement être `#installation`
  - **Dérive d'ancre entre locales** : une page traduite a renommé ses titres, mais les liens de cette locale pointent toujours vers l'ancien fragment anglais
  ...
  **Options :**
  ...
  | Flag | Description |
  |------|-------------|
  | `--dry-run` | Prévisualise les corrections prévues sans écrire de fichiers |
  | `-y, --yes` | Applique les corrections sans invite de confirmation |
  | `--types <list>` | Types d'avertissements à corriger, séparés par des virgules (par défaut : tous les types pris en charge) |

  <Card title="Guide de correction des liens rompus" icon="wrench" href="/fr/cli/fix-broken-links">
    Procédure étape par étape : prévisualiser, appliquer, réviser et valider
  </Card>
</Accordion>

<Accordion title="jamdesk spellcheck" icon="spell-check">
  Vérifie votre documentation à la recherche de fautes d'orthographe.
  ...
  **Exemple de sortie :**
  ...
  Utilise un dictionnaire anglais avec plus de 150 termes techniques intégrés (API, GraphQL, Kubernetes, React, etc.) afin que le jargon courant ne soit pas signalé. Ignore les blocs de code, le code en ligne, le frontmatter, le JSX, les URL et les chemins de fichiers. Actuellement disponible en anglais uniquement ; la prise en charge multilingue du dictionnaire est prévue.

  **Options :**

  | Flag | Description |
  |------|-------------|
  | `--fix` | Corrige les fautes de manière interactive ou les ajoute à la liste d'exclusion |
  | `--json` | Génère une sortie au format JSON (pour les pipelines CI) |
  | `-v, --verbose` | Affiche chaque fichier au fur et à mesure de la vérification |

  **Le mode de correction interactif (`--fix`)** parcourt chaque mot mal orthographié unique :
  ...
  - **Fix** remplace le mot par une suggestion dans tous les fichiers (sans risque pour le code, il ne modifie ni les blocs de code ni les attributs JSX). Jusqu'à 3 suggestions sont affichées, la meilleure correspondance étant marquée comme recommandée.
  ...
  **Liste d'exclusion personnalisée :** ajoutez des termes spécifiques à votre projet dans votre docs.json :
  ...
  Le nom de votre projet issu de `docs.json` est automatiquement exclu.
</Accordion>

<Accordion title="jamdesk openapi-check" icon="file-code">
  Valide un seul fichier de spécification OpenAPI.
...
<Note>
  **Vos spécifications OpenAPI sont validées à trois endroits.** `jamdesk dev` s'arrête au démarrage si une spécification référencée est invalide, et `jamdesk validate` / `jamdesk openapi-check` vérifient les spécifications à la demande. Lorsque vous déployez, le build cloud valide également vos spécifications référencées, mais il s'agit alors d'un **avertissement non bloquant** : le reste de votre documentation est tout de même publié, et vous êtes informé précisément du problème (une erreur d'analyse avec la ligne et la colonne, une `$ref` non résolue, ou un `operationId` en double) par e-mail et dans la liste des builds du dashboard. Corrigez la spécification et repoussez pour lever l'avertissement.
</Note>
...
<Accordion title="jamdesk rename" icon="file-pen">
  ...
  **Cela va :**
  ...
  - Mettre à jour les références de snippet
...
<Accordion title="jamdesk migrate" icon="right-left">
  ...
  Détecte votre configuration Mintlify et la convertit au format Jamdesk. Dans la même passe, elle renomme les composants dépréciés (par ex. `CardGroup` → `Columns`), déplace les fichiers MDX de snippet orphelins vers `/snippets/` et réécrit les imports relatifs au parent, extrait les composants inline qui utilisent des hooks React vers `/snippets/<name>.tsx` avec `'use client'`, et corrige automatiquement les problèmes de syntaxe MDX mécaniques. Idempotente, vous pouvez donc la relancer en toute sécurité.
...
<Accordion title="jamdesk deploy" icon="cloud-arrow-up">
  ...
  | Flag | Description |
  |------|-------------|
  | `--detach` | Met en file d'attente et quitte immédiatement |
  | `--full-rebuild` | Force un build complet (sans cache) |
  | `--project <id>` | Déploie vers un projet spécifique |
  | `--allow-empty` | Autorise le déploiement avec zéro page de contenu `.mdx` (refusé par défaut) |
...
<Accordion title="jamdesk deploy-proxy cloudflare" icon="cloud">
  ...
  Interactif par défaut : vérifie Wrangler, valide votre compte Cloudflare, détecte automatiquement votre slug à partir de `docs.json`, génère les fichiers du Worker et effectue le déploiement si vous le souhaitez. Avec `--yes`, il génère les fichiers puis s'arrête — déployez avec `npx wrangler deploy` depuis le répertoire de sortie.

  | Flag | Description |
  |------|-------------|
  | `--slug <slug>` | Slug du projet Jamdesk |
  | `--domain <domain>` | Domaine cible (par ex. `example.com`) |
  | `--path <path>` | Préfixe de chemin (par défaut : `/docs`) |
  | `--output-dir <dir>` | Répertoire de sortie (par défaut : `cloudflare-worker/`) |
  | `--skip-deploy` | Ignore l'invite « déployer maintenant ? » lors d'une exécution interactive |
  | `--force` | Écrase le répertoire de sortie s'il existe déjà |
  | `--yes` | Répond à chaque invite avec sa valeur par défaut (mode CI). Ne déploie jamais et n'écrase jamais un répertoire existant |
...
<Accordion title="jamdesk doctor" icon="stethoscope">
  ...
  **Vérifications :**
  - Version de Node.js (nécessite v20+)
  - Version de npm
...
<Accordion title="jamdesk update" icon="arrow-up">
  ...
  Vous pouvez également effectuer la mise à jour manuellement :
...
| `verbose` | boolean | false | Active la sortie détaillée par défaut |
| `checkUpdates` | boolean | true | Vérifie les mises à jour du CLI au démarrage |
...
<AccordionGroup>
  <Accordion title="Erreurs de syntaxe MDX">
    ...
    **Solutions :**
    - Utilisez `&lt;` pour un inférieur à littéral : `Values &lt;50% are low`
    - Réécrivez pour éviter le caractère : `"Below 50%"` au lieu de `"<50%"`
    - Exécutez `jamdesk validate` pour obtenir des messages d'erreur détaillés avec les numéros de ligne
  </Accordion>
  ...
  <Accordion title="docs.json introuvable">
    ...
    **Solutions :**
    ...
    - Vérifiez que le fichier s'appelle exactement `docs.json` (et non `doc.json` ou similaire)
  </Accordion>

  <Accordion title="Le serveur de développement ne démarre pas">
    Le serveur de développement peut échouer au démarrage pour plusieurs raisons.

    **Essayez ces étapes :**
    1. Exécutez `jamdesk doctor` pour vérifier votre environnement
    2. Exécutez `jamdesk clean` pour vider le cache
    3. Utilisez `jamdesk dev --verbose` pour obtenir une sortie d'erreur détaillée
  ...
  <Accordion title="Première exécution lente">
    ...
    Ceci est normal et ne se produit qu'une seule fois. Les exécutions suivantes seront beaucoup plus rapides.
  </Accordion>
  ...
  <Accordion title="Erreurs de permission refusée">
    Il se peut que vous n'ayez pas les autorisations d'écriture sur le répertoire de cache.

    **Solutions :**
    1. Vérifiez les autorisations sur `~/.jamdesk` : `ls -la ~/.jamdesk`
