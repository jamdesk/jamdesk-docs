...
**Intégrez votre changelog n'importe où.** Un lanceur « Quoi de neuf ? » s'ajoute à votre propre application avec une seule balise `<script>` et ouvre votre changelog Jamdesk dans une fenêtre modale, avec une pastille de nouveautés par visiteur. Générez le snippet depuis **Paramètres → Widget Quoi de neuf**, puis réglez le mode de lancement, le coin, la taille de la modale et la couleur de la pastille avec des attributs `data-`. Votre changelog reste dans votre documentation ; le widget l'amène dans votre produit. [Intégrer votre changelog →](/fr/development/embed-changelog)

**Téléchargez vos specs OpenAPI sous forme de zip.** Sur n'importe quelle page de référence d'API, le menu Actions IA propose désormais une option **Télécharger la spec OpenAPI** qui regroupe chaque spec OpenAPI référencée par votre site — aussi bien les fichiers locaux que les URLs distantes — dans un unique `api-specs.zip`. Les specs en plusieurs fichiers conservent leur structure de dossiers `$ref`, si bien que le téléchargement s'intègre directement dans Postman, dans un générateur de clients ou dans vos propres outils. [Actions IA →](/fr/ai/ai-actions)

**Specs OpenAPI validées au moment de la build.** Chaque déploiement valide désormais strictement les specs OpenAPI référencées par votre `docs.json` — et signale tout problème via un **avertissement non bloquant** au lieu de faire échouer la build. Votre documentation est tout de même publiée ; vous êtes simplement informé précisément du problème (erreur d'analyse YAML avec ligne et colonne, `$ref` non résolu ou `operationId` en double) par e-mail et dans la liste des builds du tableau de bord. En local, `jamdesk dev` s'arrête toujours sur une spec invalide pour que vous la détectiez avant de pousser. [Aperçu de la CLI →](/fr/cli/overview)
</Update>
