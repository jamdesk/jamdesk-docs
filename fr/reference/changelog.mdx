---
title: Changelog
description: "Nouveautés de Jamdesk : fonctionnalités, mises à jour CLI et changements de plateforme pour MDX, GitHub et la documentation API."
rss: true
---
...
**Aperçus sociaux personnalisés depuis le frontmatter.** Définissez n'importe quelle balise meta Open Graph ou Twitter/X directement dans le frontmatter d'une page — `og:title`, `og:description`, `og:image`, `twitter:card`, `keywords`, ainsi que l'ensemble complet des metatags. Utilisez des clés plates de premier niveau (style Mintlify) ou un bloc `seo:` imbriqué ; les deux fonctionnent, et les valeurs par page remplacent les valeurs par défaut de votre `docs.json`. [SEO →](/fr/content/seo)
