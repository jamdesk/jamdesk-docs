...
**Intégrez votre changelog n'importe où.** Un lanceur « Quoi de neuf ? » s'ajoute à votre propre application avec une seule balise `<script>` et ouvre votre changelog Jamdesk dans une fenêtre modale, avec une pastille de nouveautés par visiteur. Générez le snippet depuis **Paramètres → Widget Quoi de neuf**, puis réglez le mode de lancement, le coin, la taille de la modale et la couleur de la pastille avec des attributs `data-`. Votre changelog reste dans votre documentation ; le widget l'amène dans votre produit. [Intégrer votre changelog →](/fr/development/embed-page)
