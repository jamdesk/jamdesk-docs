...
Si votre site est multilingue, déposez un fichier `<spec>.<lang>.<ext>` à côté de chaque spec source (par exemple, `openapi/api.fr.yaml`) et Jamdesk le sert sur les URL de cette langue. Voir [Traduction des specs OpenAPI](/fr/setup/languages#translating-openapi-specs).
...
Activez la protection par mot de passe partagé pour votre site. Configuration déclarative uniquement. Vous définissez toujours la phrase de passe réelle dans le [dashboard](/fr/setup/password-protection#rotating-and-revoking-sessions) après l'exécution du prochain build.
