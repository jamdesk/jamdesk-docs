...
| Champ | Type | Défaut | Description |
|-------|------|---------|-------------|
| `autoRedirect` | boolean | `false` | Quand `true`, un visiteur qui arrive pour la première fois sur la racine de votre documentation est envoyé vers la langue correspondant à l'en-tête `Accept-Language` de son navigateur. Nécessite deux entrées ou plus dans `navigation.languages`. |

Seules les racines sont redirigées. Un lien profond comme `/guides/authentication` sert toujours la page qu'il désigne, donc un lien que vous collez dans un ticket ouvre la même page pour tout le monde. Les crawlers des moteurs de recherche ne sont jamais redirigés, vos balises `hreflang` continuent donc de décider de ce qui est indexé.

La langue du visiteur est mémorisée pendant un an. Choisir une langue dans le sélecteur remplace la correspondance automatique à partir de ce moment.

Voir [Support multi-langue → Routage automatique de la langue](/fr/setup/languages#routage-automatique-de-la-langue) pour le comportement complet.
