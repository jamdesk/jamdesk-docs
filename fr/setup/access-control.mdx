...
Jamdesk vous offre quatre façons de contrôler qui peut lire vos docs. La plupart des équipes en choisissent une et s'y tiennent ; certaines combinent les approches. Cette page présente les options, puis vous oriente vers le guide détaillé correspondant.
...
La demande la plus fréquente : « Je veux des docs internes étendues protégées par une connexion, et un site public plus restreint pour les clients. » Vous n'avez pas besoin de deux projets pour cela. Utilisez le **mode mixte** dans un seul projet Jamdesk :
...
On confond parfois trois concepts d'accès distincts. Voici la séparation claire :
...
Un membre de l'équipe peut avoir un, deux ou les trois rôles — ce sont des axes indépendants.
