...
description: Gardez la documentation aux côtés de votre code. Jamdesk prend en charge les monorepos et tout dépôt où les docs ne se trouvent pas à la racine.
---
...
<Note>
**Prérequis :** Vous avez besoin d'un [projet Jamdesk](/fr/setup/creating-projects) connecté à un [dépôt GitHub](/fr/setup/connecting-github) avant de configurer le support des monorepos.
</Note>
...
<Steps>
  ...
  <Step title="Entrer le chemin de vos docs">
...
- **Les assets** dans les sous-répertoires (comme `images/`) sont inclus
...
- Les modifications dans `packages/docs/**` déclenchent un build
- Les modifications dans `packages/core/**` ne déclenchent pas de build
...
<AccordionGroup>
  <Accordion title="Erreur docs.json introuvable" icon="circle-exclamation">
    ...
    4. N'oubliez pas : utilisez `docs` et non `docs/docs.json`

    **Vérification rapide :** Dans votre dépôt, le fichier doit exister à `[your-docs-path]/docs.json`
  </Accordion>

  <Accordion title="Assets qui ne se chargent pas" icon="image">
  ...
  <Accordion title="Les modifications ne déclenchent pas de builds" icon="rotate">
    ...
    3. Consultez le statut de livraison des webhooks dans les paramètres du dépôt GitHub
