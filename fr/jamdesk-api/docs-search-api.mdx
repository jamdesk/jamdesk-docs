...
description: Interrogez votre documentation Jamdesk par programmation. Alimentez chatbots, bots Slack, recherches personnalisées et agents IA avec des réponses à jour.
---
...
| **Renvoyé dans la réponse** | Chaque réponse 200 inclut un champ `language` avec la valeur résolue par le serveur (valeur de la requête, ou `en` par défaut). |

<Info>
Un projet est multilingue lorsque son `docs.json` contient un tableau `navigation.languages` avec deux entrées ou plus. Pour vérifier, ouvrez l'onglet **Settings → Languages** dans le tableau de bord, ou consultez `docs.json` directement.
</Info>
