---
title: API de recherche de documentation
description: Recherchez votre documentation Jamdesk par programmation avec l'API de recherche de documentation. Alimentez des chatbots de support, des bots Slack, des recherches personnalisées et des agents IA avec des réponses précises et à jour issues de votre documentation.
---
...
Il n'existe pas de SDK officiels pour les langages à ce stade. Utilisez l'API REST directement via `fetch`, `requests`, `curl` ou tout client HTTP. La [collection Postman](#postman-collection) ci-dessous fournit des exemples prêts à être dupliqués.
