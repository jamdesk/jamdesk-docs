...
L'API est actuellement à la version **v1.0.0**. Les changements incompatibles (renommages de champs, endpoints supprimés, authentification modifiée) seront annoncés via le [blog Jamdesk](https://jamdesk.com/blog) et un avis de dépréciation dans l'en-tête de réponse `X-Deprecation` au moins 90 jours avant la suppression.
