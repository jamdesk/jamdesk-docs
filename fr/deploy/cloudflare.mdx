...
Avec `--yes`, la CLI ne peut pas afficher d'invite : elle doit donc connaître votre sous-domaine. Elle le lit depuis le projet lié dans `docs.json` ; si ce lien est absent (lancez `jamdesk deploy` une fois pour l'ajouter), passez `--slug` explicitement, sinon la commande s'arrête au lieu de deviner.
