...
description: "Redirigez les requêtes /docs vers votre documentation Jamdesk via un Cloudflare Worker. Configuration du Worker, patterns de routes et cache."
---
...
Pour un déploiement CI ou scripté, utilisez des flags pour ignorer les invites :

```bash
jamdesk deploy-proxy cloudflare --slug myproject --domain example.com --skip-deploy --yes
```

Avec `--yes`, le CLI ne peut pas afficher d'invite : elle doit donc connaître votre sous-domaine. Elle le lit depuis le projet lié dans `docs.json` ; si ce lien est absent (lancez `jamdesk deploy` une fois pour l'ajouter), passez `--slug` explicitement, sinon la commande s'arrête au lieu de deviner.
...
Jamdesk sert le HTML de la documentation avec `Cache-Control: no-store`, donc Cloudflare ne met pas les pages en cache à l'edge (`cf-cache-status: BYPASS`). Chaque requête rend la version actuelle, et les changements publiés apparaissent immédiatement, sans délai de cache.

Les ressources statiques sous `/_next/` et `/_jd/` (JavaScript, CSS, polices, images) sont servies avec des en-têtes de cache `immutable` de longue durée, donc Cloudflare les met en cache à l'edge. Leurs noms de fichiers sont hachés selon leur contenu : chaque build produit de nouvelles URL et les ressources mises à jour sont prises en compte automatiquement. Aucune purge requise.

`cacheEverything: true` permet à Cloudflare de mettre en cache ces ressources statiques sur la route proxifiée ; cela n'annule pas le `no-store` du HTML. Pour vider le cache edge manuellement, utilisez la fonctionnalité **Purge Cache** de Cloudflare (Caching → Configuration → Purge Everything).
</Accordion>
