---
title: Cursor
description: "Configurez Cursor pour rédiger la documentation Jamdesk : règles MDX, autocomplétion des composants et accès MCP pour le contexte des docs en direct."
---

[Cursor](https://cursor.com) est un éditeur de code IA basé sur VS Code. Son édition en ligne (Cmd+K) et son chat (Cmd+L) fonctionnent bien pour la rédaction MDX itérative : sélectionnez un paragraphe, décrivez ce que vous voulez, et Cursor le réécrit sur place.
...
<Steps>
  ...
  <Step title="Ajoutez des règles Cursor">
    Créez `.cursor/rules/jamdesk-docs.mdc` avec des standards de documentation pour que l'IA de Cursor respecte vos conventions.
  </Step>
  <Step title="Connectez le serveur MCP">
    Ajoutez l'endpoint MCP de vos docs à `.cursor/mcp.json` pour que Cursor puisse rechercher dans votre documentation publiée.
  </Step>
</Steps>
...
Créez `.cursor/mcp.json` pour connecter vos docs publiées :
...
Remplacez `your-project` par votre sous-domaine Jamdesk — ou, si vous disposez d'un domaine personnalisé actif, utilisez-le à la place : `"url": "https://docs.acme.com/_mcp"`. Consultez [Serveur MCP](/fr/ai/mcp-server) pour les détails sur l'endpoint.

## Workflows

<Tabs>
  <Tab title="Édition en ligne (Cmd+K)">
    Sélectionnez du texte dans un fichier MDX et appuyez sur **Cmd+K** pour décrire ce que vous voulez modifier. Fonctionne bien pour :
    - Réécrire une section pour plus de clarté
    - Ajouter un exemple de code
    - Convertir du texte en composant (par exemple, envelopper des étapes dans `<Steps>`)
  </Tab>
  <Tab title="Chat (Cmd+L)">
    Ouvrez le chat avec **Cmd+L** et posez des questions en utilisant vos docs comme contexte :
    - « Quels composants sont disponibles pour cette page ? »
    - « Rédige une section de dépannage pour [fonctionnalité] »
    - « Vérifie cette page par rapport à nos standards de rédaction »
  </Tab>
  <Tab title="Composer">
    Utilisez Composer pour des tâches de documentation multi-fichiers :
    - Générer une nouvelle section avec plusieurs pages
    - Restructurer la navigation entre les fichiers
    - Mettre à jour en masse les modèles d'utilisation des composants
  </Tab>
</Tabs>

## Le skill /update-jamdesk

Pour des mises à jour automatisées de la documentation lors des changements de code, installez le skill `/update-jamdesk` :
