---
title: Cursor
description: "Configurez Cursor IDE pour rédiger la documentation Jamdesk avec des règles MDX, l'autocomplétion des composants et l'accès au serveur MCP."
---

[Cursor](https://cursor.com) est un éditeur de code IA basé sur VS Code. Son édition en ligne (Cmd+K) et son chat (Cmd+L) fonctionnent bien pour la rédaction itérative en MDX : sélectionnez un paragraphe, décrivez ce que vous voulez, et Cursor le réécrit sur place.
...
<Steps>
  ...
  <Step title="Connectez le serveur MCP">
    Ajoutez le endpoint MCP de votre documentation à `.cursor/mcp.json` pour que Cursor puisse rechercher dans votre documentation publiée.
  </Step>
</Steps>
...
Remplacez `your-project` par votre sous-domaine Jamdesk — ou, si vous avez un domaine personnalisé actif, utilisez-le à la place : `"url": "https://docs.acme.com/_mcp"`. Consultez [Serveur MCP](/fr/ai/mcp-server) pour les détails du endpoint.

## Workflows

<Tabs>
  <Tab title="Édition en ligne (Cmd+K)">
    Sélectionnez du texte dans un fichier MDX et appuyez sur **Cmd+K** pour décrire ce que vous voulez changer. Fonctionne bien pour :
  ...
  <Tab title="Chat (Cmd+L)">
    ...
    - « Rédiger une section de dépannage pour [fonctionnalité] »
    - « Vérifier cette page par rapport à nos standards de rédaction »
  </Tab>
  <Tab title="Composer">
