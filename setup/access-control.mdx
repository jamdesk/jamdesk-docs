...
Jamdesk gives you four ways to control who can read your docs. Most teams pick one and stick with it; some mix and match. This page frames the decision, then points you at the deep-dive guide for whichever option you pick.
...
The most common ask: "I want extensive internal docs locked behind a login, plus a smaller public site for customers." You don't need two projects for this. Use **mixed mode** in a single Jamdesk project:
...
For larger internal sections, list the paths in `docs.json` instead of marking each file. Note: `auth.password.private[]` activates specific-pages mode automatically — you don't add `enabled: true` (that's whole-site mode, the opposite of what you want here).
...
People sometimes confuse three different access concepts. Here's the clean split:
...
A teammate can be one, two, or all three — they're independent axes.
