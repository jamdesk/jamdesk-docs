...
description: "Choose how readers reach your docs: fully public, password-gated, mixed public/private, your own login via JWT, or SSO. Pick the one that fits your audience."
---

Jamdesk gives you five ways to control who can read your docs. Most teams pick one and stick with it; some mix and match.
...
| **Your own login (JWT)** | Readers already sign in to your product. Your backend signs a short-lived token for each of them, Jamdesk turns it into a session, and pages can be limited to groups you name in the token. | `auth.jwt.enabled: true` plus a `loginUrl` in `docs.json`, and a signing key from the dashboard. Included on every plan. See [JWT Authentication](/setup/jwt-authentication). |
...
### Per-user access through your own login

If your customers already have accounts with you, a shared password is a step backwards: it gets forwarded, it never expires on its own, and it can't tell one customer from another. With [JWT authentication](/setup/jwt-authentication), a signed-in user who opens your docs is bounced to a URL on your side, your backend signs a token, and Jamdesk creates a session for that person. Readers need no Jamdesk account, and there is no shared secret to forward around.

The token can also carry groups. Add `groups: ["admin"]` to a page's frontmatter and only visitors whose token lists `admin` can open it, or see it in the navigation. Everyone else gets a 404, so the page doesn't give away that it exists.
...
Pair it with `public` paths for the parts of the site that should stay open, such as a changelog or a status page.
...
| **Readers** | View the published docs site. | Everyone (public), anyone with the password (password mode), anyone your login flow signs a token for (JWT), or anyone authenticated via your IdP (SSO). |
...
<Columns cols={2}>
  ...
  <Card title="JWT Authentication" icon="id-badge" href="/setup/jwt-authentication">
    Gate the docs behind your own login, with per-user sessions and group-restricted pages.
  </Card>
