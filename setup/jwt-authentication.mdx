...
- To restrict a page to nobody, unpublish it. There's no `groups` value that means "nobody": group membership is additive (any overlap grants access), so an empty or unknown group list is equivalent to no restriction.
