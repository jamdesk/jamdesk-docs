...
This page is for anyone who wants their docs read in more than one language without running a translation vendor or a script of their own. For how per-locale navigation and the language switcher work, see [Multi-Language Support](/setup/languages) — that configuration applies whether the translations come from Jamdesk, your own translators, or another tool.
...
Automatic translation is included on every plan, trials included. There's no per-word pricing and no credits to buy.
...
Two limits apply, both per project:
...
Because only changed pages are translated again after the first run, a settled site uses a small fraction of the monthly allowance.

## Only what changed

The first run translates everything. After that, Jamdesk retranslates only the pages whose English source actually changed, so fixing a typo on one page costs one page rather than your whole site.
...
- **Translations you already had.** If your repository already contains translated pages when you switch the feature on, Jamdesk adopts them rather than overwriting them. Work you wrote yourself, or paid a vendor for, is kept exactly as it is.
...
Every page listed there has a **Replace with AI translation** button. It hands that page back to the AI — your version is discarded and the page is translated again on your next build, so nothing changes the moment you click. That button is the only thing that lifts the hold.
