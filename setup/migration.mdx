...
<Tabs>
  <Tab title="From Mintlify">
    ...
    **Jamdesk (`docs.json`):**
    ...
    <AccordionGroup>
      <Accordion title="CardGroup needs to be replaced with Columns">
        Mintlify's `<CardGroup cols={2}>` becomes `<Columns cols={2}>` in Jamdesk. The `jamdesk migrate` CLI handles this, but check any files you've edited manually.
  ...
  <Tab title="From Other Platforms">
    For GitBook, Docusaurus, Confluence, Notion, or other documentation tools.
