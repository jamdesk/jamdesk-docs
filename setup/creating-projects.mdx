...
Before writing any docs you need a project. You can start from a template (Jamdesk creates the repo for you, on GitHub or Bitbucket Cloud) or connect an existing repository that already has a `docs.json`. Either way, you will have a live site within a few minutes.
  ...
  <Step title="Choose a starting point">
    <Tabs>
      ...
      <Tab title="From Repository">
        ...
        Repository on Bitbucket Cloud? Enter it as `workspace/repository-slug` instead. See [Connect Bitbucket](/setup/connecting-bitbucket).
      </Tab>
    </Tabs>
  </Step>
...
- **Host at a subpath** - Serve docs at a path like `/docs` or `/help` instead of the site root
