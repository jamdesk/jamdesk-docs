...
description: "Jamdesk uses two GitHub connections: a GitHub App for repository access and a personal account link for build attribution."
---

Jamdesk uses **two separate GitHub connections**, each with a different job. This page explains what each connection does and when you need it.
...
<Note>
Bitbucket Cloud projects work differently: a single OAuth authorization covers repository access, the webhook, and build attribution. See [Connect Bitbucket](/setup/connecting-bitbucket).
</Note>
...
When you set up a project, Jamdesk walks you through connecting GitHub. There are two paths.

#### Create a starter docs repo (default for new projects)

<Steps>
  <Step title="Sign in with GitHub">
    Authorize Jamdesk so it can create a repository on your account or organization.
  </Step>
  <Step title="Choose where to create it">
    Pick your personal account or an organization. Jamdesk creates a new repository pre-filled with starter documentation.
  </Step>
  <Step title="Install the GitHub App">
    Install the Jamdesk GitHub App (or grant it access to the new repo) so it can build and auto-deploy your docs.
  </Step>
  <Step title="Push triggers builds">
    Every push to your configured branch automatically triggers a deployment. Customize the starter docs whenever you're ready.
  </Step>
</Steps>

#### Connect an existing repo

Already have a docs repository? Install the Jamdesk GitHub App, grant it access to that repository, then choose the repo and branch in Jamdesk under **Project Settings → Git Repository**. From there it works the same way.
...
Use the same email that appears on your Jamdesk account to avoid "Unknown user" attribution in build history.
...
Each action removes a different piece of access. Here is what each one affects.
...
If you've previously installed the Jamdesk GitHub App, clicking "Install GitHub App" takes you to GitHub's settings page instead of the installation page. This is normal! Here's how to complete setup:
...
<Steps>
  ...
  <Step title="Enable the Save button">
    If the Save button is disabled (grayed out), switch from "All repositories" to "Only select repositories", then switch back.
  </Step>
  <Step title="Click Save">
    Click the green **Save** button. Jamdesk detects the change and continues setup.
  </Step>
</Steps>

### Popup blocked

If the GitHub popup doesn't open, your browser may have blocked it. Look for a popup blocker icon in your address bar, allow popups from Jamdesk, then click **Try Again**.
...
Your company probably manages your GitHub account, and GitHub doesn't let these accounts install apps on their own account. See [company-managed GitHub accounts](/help/troubleshooting/github-issues#company-managed-github-accounts) to fix it.
...
<Columns cols={2}>
  ...
  <Card title="Troubleshooting Builds" icon="wrench" href="/help/troubleshooting/build-failures">
