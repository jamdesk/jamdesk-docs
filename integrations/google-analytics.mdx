...
description: >-
  Add Google Analytics 4 to your documentation site using a measurement ID to
  track visitors and understand content engagement.
---
...
You are responsible for setting up cookie consent banners for any tags you load through Google Tag Manager if you need them. Jamdesk loads your GTM container for all visitors without a consent gate. See [Cookie Consent](/integrations/cookie-consent) to gate analytics until visitors opt in.

<Warning>
Don't add the same GA4 property both directly in `docs.json` and through Google Tag Manager, because this will cause duplicate tracking.
</Warning>
...
- Adding a cookie consent banner if GDPR/CCPA applies. Jamdesk loads GA4 for every visitor by default, so use [Cookie Consent](/integrations/cookie-consent) to gate it until they opt in.
