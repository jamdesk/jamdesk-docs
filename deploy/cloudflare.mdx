...
<Note>
Using a custom subpath from the dashboard instead of the default `/docs`? Replace `"/docs"` in `PROXY_PATHS` — and in the exact-match check inside `shouldProxy()` — with your configured subpath. The CLI's `--path` flag does this for you when it generates the file, but only against an **unmodified** template; regenerating overwrites any custom entries you've added to `PROXY_PATHS` by hand. If you've customized this Worker, edit the `/docs` entries in place instead.
</Note>
