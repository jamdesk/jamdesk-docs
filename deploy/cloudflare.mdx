...
description: Proxy /docs requests through a Cloudflare Worker to your Jamdesk site.
---
...
1. **Missing routes in `wrangler.toml`** — The `routes` array needs patterns for `/_next/*` and `/_jd/*` in addition to `/docs*`. Without them, asset requests bypass the Worker.
2. **Missing `X-Forwarded-Host` header** — The Worker must set this to your domain so Jamdesk generates correct asset URLs.
</Accordion>
