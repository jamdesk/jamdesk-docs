...
description: Use Cloudflare Workers to proxy documentation requests from your domain to Jamdesk. Workers run at the edge, providing fast response times globally.
---
...
routes = [
  ...
  { pattern = "yoursite.com/_next/*", zone_name = "yoursite.com" },
  { pattern = "yoursite.com/_jd/*", zone_name = "yoursite.com" },
]
```

<Note>
The `/_next/` and `/_jd/` routes are required for Next.js assets and Jamdesk resources (fonts, images, branding) to load correctly.
</Note>
