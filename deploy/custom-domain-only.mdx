...
|---------|----------------|
| *"We couldn't reach your docs through your domain. Your proxy needs to forward `/_jd/*` to Jamdesk — add a rewrite for `/_jd/:path*` and try again."* | Your domain either isn't proxying at all, or its proxy doesn't forward `/_jd/*`. Preflight lives there specifically so it stays reachable through every proxy template Jamdesk generates. |
