...
<Note>
Toggling "Host at /docs" on or off, or changing the subpath, triggers an automatic rebuild since the URL structure changes.
</Note>
...
<Warning>
Removing a domain that had "Host at /docs" enabled will trigger a rebuild to update URL paths. Any custom subpath is cleared along with the domain, so if you reconnect a domain later, docs start back at the default `/docs`.
</Warning>
