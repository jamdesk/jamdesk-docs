...
### Host at /docs (Proxy-Based, Customizable Subpath)
...
`/docs` is the default; you can choose any single-segment subpath instead (`/help`, `/guide`, and so on) from the same toggle. You configure a proxy (Cloudflare, Vercel, nginx, etc.) to forward requests for your chosen subpath to Jamdesk. See [Subpath Hosting](/deploy/subpath-hosting) for setup guides and the rules for choosing a subpath.

<Note>
Toggling "Host at a subpath" on or off, or changing the subpath, triggers an automatic rebuild since the URL structure changes.
</Note>
...
You do not need to configure certificates manually.
...
<Warning>
Removing a domain that had "Host at a subpath" enabled will trigger a rebuild to update URL paths. Any custom subpath is cleared along with the domain, so if you reconnect a domain later, docs start back at the default `/docs`.
</Warning>
...
## Does the jamdesk.app subdomain compete with my domain in search?

No. Once your custom domain is active, pages served directly from your
`[slug].jamdesk.app` subdomain include a canonical link pointing at your
domain, so search engines treat your domain as the single source of truth
and consolidate all ranking signals there.
