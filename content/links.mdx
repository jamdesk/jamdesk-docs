...
When a broken link has an unambiguous correct target (a typo'd anchor, or cross-locale anchor drift where a translated page renamed its headings), `jamdesk fix` repairs it for you:
...
It only rewrites a link when the corrected anchor is a real heading on the target page, leaving anything ambiguous for manual review. See [Auto-fix broken links](/cli/fix-broken-links).
