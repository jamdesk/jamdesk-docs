...
Embed `.mp4` and `.webm` video files directly in your docs — no external service needed. Place video files in a `/videos` directory in your project root, then reference them using markdown or the `<Video>` component.

#### Markdown syntax

The simplest approach — works just like images:

```markdown
![Product demo](/videos/sample-animation.mp4)
```

![Product demo](/videos/sample-animation.mp4)

This renders an HTML5 video player with default controls (play, pause, seek, volume, fullscreen).

#### Video component

For more control over playback behavior, use the `<Video>` component:

```mdx
<Video src="/videos/sample-animation.mp4" title="Sample animation" />
```
...
For short looping clips (like UI demos), use autoplay with mute and no controls:

```mdx
<Video src="/videos/sample-animation.mp4" autoPlay muted loop controls={false} />
```
