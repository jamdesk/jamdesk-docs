...
title: Videos
description: >-
  Learn how to embed MP4 and WebM videos in Jamdesk using Markdown or the Video
  component, with props for autoplay, loop, and controls.
---
...
|------|------|---------|-------------|
| `src` | `string` | none | **Required.** Path to the video file. |
| `title` | `string` | none | Accessibility label for screen readers. |
...
| `width` | `string` | none | CSS width value (e.g., `"400px"`, `"100%"`). |
| `height` | `string` | none | CSS height value. |
...
Video files can be up to **100MB**. For anything longer than a few minutes, host it on YouTube and use the [`<YouTube>` component](/content/youtube) instead, since streaming from a CDN built for video will always outperform serving large files from your docs.
