...
When the built-in components are not enough, export an arrow-function component at the top of any MDX file and use it inline. These components are server-rendered at build time, have access to all Jamdesk built-ins, and can be styled with Tailwind classes.
