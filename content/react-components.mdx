...
- **Layout:** `Card`, `CardGroup`, `Tabs`, `Tab`, `Accordion`, `Steps`, `Step`, `Columns`, `View`
...
- **No hooks**: React hooks like `useState` and `useEffect` will cause runtime errors (components are server-rendered)
