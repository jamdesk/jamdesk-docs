...
Add `nocopy` to drop the copy button from a single block:
...
Use it wherever copying the block would hand the reader something that doesn't work: a terminal transcript with prompts and output mixed together, a redacted example, or a "don't do this" snippet.

`nocopy` combines with every other option:
