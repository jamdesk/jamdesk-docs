...
| `search` | boolean | `true` | Set to `false` to keep the page out of site search, AI chat answers, and MCP. It stays in your sidebar, sitemap, and llms.txt. See [Keep a page out of search only](/content/hidden-pages#keep-a-page-out-of-search-only) |
...
| `public` | boolean | `false` | Exempt this page from password protection or [JWT authentication](/setup/jwt-authentication) when the whole site is gated. Wins over `private: true` if both are set. |
| `groups` | string[] | - | With JWT authentication on, only visitors whose token lists at least one of these groups can open the page; everyone else gets a 404 and doesn't see it in the navigation. Up to 32 groups per page. Ignored in password mode. See [Group-based access](/setup/jwt-authentication#group-based-access). |
| `lastUpdatedDate` | string | - | Override the "Last updated on" date for this page instead of using its last Git commit date. Quote the value: `"2026-09-01"`. The footer line needs `metadata.timestamp` in `docs.json`; your sitemap and structured data use the date either way. See [Last updated date](#last-updated-date) |

### Last updated date

With `metadata.timestamp` enabled in `docs.json`, every page shows the date of the last commit that touched its file. `lastUpdatedDate` replaces that date on one page:
...
Use it when you've reviewed a page and want to say so, or when a formatting commit moved a date without changing anything a reader would notice.

Quote the value. An unquoted `2026-09-01` is read as a timestamp rather than a date and can display a day off. A value that isn't a date is ignored and the commit date shows instead, so a typo never blanks the line.

<Note>
The footer line only appears with `metadata.timestamp` enabled, but `lastUpdatedDate` isn't only a footer setting: your sitemap's `<lastmod>` and the page's structured-data `dateModified` both use it whether or not you've turned the footer on. Set it when the date is what you mean, not only when you want it displayed.
</Note>

The override also sets `dateModified` in the page's structured data, so search engines and AI assistants see the same date your readers do.
...
<Tip>
After a build, paste the page URL into the free [OpenGraph Preview](https://jamdesk.com/utilities/opengraph-preview) tool to see how these tags render on X, Facebook, LinkedIn, Slack, Discord, and more.
</Tip>
...
Paste the block between the `---` markers into the free [YAML Validator](https://jamdesk.com/utilities/yaml-validator) to pinpoint the line and column of the error.
</Accordion>
