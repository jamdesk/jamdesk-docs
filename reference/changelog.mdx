...
**Auto-detect `docs.json` on connect.** Connect a GitHub repo and Jamdesk scans the tree for your `docs.json`, so a config in a subdirectory or monorepo path works without manual setup. If it finds a Mintlify `mint.json` instead, the Connect card tells you and points at [migration](/setup/migration). Detection errors show inline on the card, not buried in a build log. [Connecting GitHub →](/setup/connecting-github)

**`docs.json` location hints in the CLI.** When `jamdesk validate`, `jamdesk dev`, or `jamdesk doctor` can't find your `docs.json`, they now tell you where it actually is (or where it should go) instead of failing with a generic error. Useful when you're in a monorepo or migrating a project with the config nested a few levels down. [CLI overview →](/cli/overview)

**Standards-compliant hreflang for multi-language sites.** Hreflang tags now emit valid BCP 47 codes. Locale folders like `cn`, `jp`, or `ja-jp` map to the codes search engines expect (`zh`, `ja`), and sitemap URLs are XML-escaped. If you run a translated site, crawlers read the language alternates correctly. [Languages →](/setup/languages)
</Update>
