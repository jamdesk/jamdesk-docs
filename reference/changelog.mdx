...
**Background customization.** A new `background` block in `docs.json` lets you disable the Jam light-mode gradient, override the page color per mode, or tune the gradient's color, size, position, and opacity. Useful for flat-look sites, brand-color backgrounds, or subtle gradient adjustments without writing custom CSS. [Background →](/customization/background)
