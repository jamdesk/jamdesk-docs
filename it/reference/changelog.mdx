...
<Update label="Settembre 2026" date="2026-09-21">
**Porta i lettori direttamente nella loro lingua.** Imposta `"localization": { "autoRedirect": true }` in `docs.json` e un visitatore alla prima visita il cui browser richiede il francese arriva su `/fr` invece che nella tua lingua predefinita. La sua scelta resta valida per un anno e selezionare una lingua dal selettore la sostituisce. Viene reindirizzata soltanto la radice del sito: un URL di lingua come `/fr` e un link profondo come `/guides/authentication` si aprono esattamente come richiesti, per tutti, quindi un link che incolli in un ticket apre la stessa pagina per chi lo clicca. Nemmeno i crawler vengono mai reindirizzati, quindi i tag `hreflang` continuano a decidere cosa viene indicizzato. Disattivato per impostazione predefinita; richiede due o più lingue configurate. [Routing automatico della lingua →](/it/setup/languages)

**Sostituisci la data di "ultimo aggiornamento" di una pagina.** Il `metadata.timestamp` di `docs.json` mostra già una riga "Last updated on …" ricavata dall'ultimo commit Git della pagina. Aggiungi `lastUpdatedDate: "2026-09-01"` al frontmatter di una pagina e quella data sostituisce nel piè di pagina la data del commit. Alimenta anche il `<lastmod>` della sitemap e il `dateModified` dei dati strutturati della pagina, che il piè di pagina sia attivo o meno, così i motori di ricerca e gli assistenti AI vedono la stessa data dei tuoi lettori. Utile dopo aver riletto una pagina, o quando un commit di sola formattazione ha spostato la data senza cambiare nulla che un lettore noterebbe. [Frontmatter →](/it/content/frontmatter)

**Disattiva il pulsante di copia su un blocco di codice.** Aggiungi `nocopy` al blocco e quel blocco perde il pulsante di copia mentre tutti gli altri lo mantengono; si combina con un titolo e con tutte le altre opzioni e funziona scheda per scheda dentro un `<CodeGroup>`. Usalo per le trascrizioni del terminale, gli esempi oscurati e gli snippet "da non fare": ovunque copiare il testo renderizzato consegnerebbe al lettore qualcosa che non funziona. [Blocchi di codice →](/it/content/code-blocks)
