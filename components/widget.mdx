...
`<Widget>` drops a live What's New trigger straight into a docs page. Click it and your Jamdesk changelog opens in the same modal your customers see when you [embed the widget in your app](/development/embed-page). The difference: here you write one MDX tag instead of a `<script>`, and the page resolves your site automatically.
...
<Note>
This button opens the modal on the deployed docs site, where `<Widget>` binds to your canonical `*.jamdesk.app` origin. In local `jamdesk dev` preview there's no published origin to load the modal from, so the trigger can't open. Instead it shows a "Live preview" caption beside the button. Deploy the page (or open it on your live site) to try the real thing.
</Note>
