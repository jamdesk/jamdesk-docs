---
title: Visibility
description: Show different content to human readers and AI agents on the same page — agent-only context, instructions, or definitions that stay out of the rendered docs.
---
...
The tip is reassuring to a human reading the docs but useless to an agent generating code — keep it out.
...
Jamdesk decides which audience is viewing based on **URL shape and Accept header**, not user-agent sniffing:

- **HTML render** (any canonical URL, e.g. `/guides/auth`) → audience: `humans`
- **`.md` URL** (e.g. `/guides/auth.md`) → audience: `agents`
- **`Accept: text/markdown` on a canonical URL** → audience: `agents`
- **`llms-full.txt`** (built into every site) → audience: `agents`

Any agent that sets the `Accept: text/markdown` header gets the agent content automatically — no URL surgery required.

## Rules & gotchas

<Warning>
  **Code blocks are safe.** The filter detects fenced (triple-backtick or triple-tilde) and inline (single-backtick) code blocks and leaves `<Visibility>` tags inside them untouched. That's why the examples above — which contain `<Visibility>` tags as *content* — render correctly.
</Warning>
...
<Note>
  **Site search includes both audiences' content** in this release. A reader could search for a term that only appears in a `<Visibility for="agents">` block and hit a result. This is a known v1 limitation — filtering search is on the roadmap.
</Note>
...
- **To A/B test content for different users.** There are only two audiences — humans and agents — and Jamdesk does not detect which specific human is viewing. Use feature flags or route redirects for user-segmented content.
