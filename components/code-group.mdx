...
## Hiding the Copy Button on a Tab

Add `nocopy` to a fence to drop the copy button on just that tab — the other tabs in the group keep theirs:
...
Use it for a tab that shouldn't be copied as-is, like a cURL example with a placeholder token, while leaving working examples in the other tabs copyable. See [Code Blocks → Hiding the Copy Button](/content/code-blocks#hiding-the-copy-button) for the flag on a standalone block.
