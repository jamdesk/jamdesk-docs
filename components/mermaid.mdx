...
description: Mermaid lets you create diagrams and visualizations using text-based syntax. Use the examples to add it to your MDX pages and keep layouts consistent.
---

Mermaid lets you create diagrams and visualizations using text-based syntax. Instead of using image editors or drawing tools, you write simple text that Mermaid converts into professional diagrams. This makes diagrams easy to version control, update, and maintain alongside your documentation.
...
Mermaid supports many diagram types, each designed for specific use cases. Choose the right diagram type to communicate your ideas effectively.

### Flowcharts

Flowcharts visualize processes, workflows, and decision trees. Use them to document business logic, user flows, or any step-by-step procedure. The direction can be top-down (`TD`), left-right (`LR`), bottom-up (`BT`), or right-left (`RL`).
...
- **Consistent direction** — Use `TD` (top-down) for tall diagrams, `LR` (left-right) for wide ones
- **Limit nodes** — Aim for 10-15 nodes maximum per diagram for clarity
