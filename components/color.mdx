...
|------|------|---------|-------------|
| `variant` | string | compact | Layout style: `compact` (grid) or `table` (grouped rows) |
| `children` | node | - | `ColorItem` or `ColorRow` elements |

### ColorRow

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `title` | string | - | Label displayed above the row |
| `children` | node | - | `ColorItem` elements |
...
| `value` | string \| object | - | Color value as CSS string, or `{ light: string, dark: string }` for theme-aware colors |
