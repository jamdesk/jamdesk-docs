...
description: "Enruta /docs a través de un Cloudflare Worker hacia tu sitio de documentación de Jamdesk. Cubre configuración del Worker, patrones de rutas y caché."
---

Un [Cloudflare Worker](https://workers.cloudflare.com/) intercepta las solicitudes a `/docs` en tu dominio, las reescribe hacia tu subdominio de Jamdesk y devuelve la respuesta, todo en el edge sin necesidad de un servidor de origen. Puedes generar el Worker automáticamente con `npx jamdesk deploy-proxy cloudflare` o configurarlo manualmente a continuación.

## Cómo funciona

El Worker reenvía las solicitudes a tu subdominio de Jamdesk y transmite el dominio original en el encabezado `X-Jamdesk-Forwarded-Host`. Jamdesk usa este encabezado para:
...
Esto significa que el Worker es una **configuración única**. Si más adelante cambias tu dominio personalizado o la configuración en el dashboard de Jamdesk, no es necesario actualizar el Worker: todas las decisiones de enrutamiento se toman del lado del servidor.

## Requisitos previos

- Una cuenta de Cloudflare con tu dominio configurado
- [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/install-and-update/) v3.0 o superior instalado
- Tu subdominio de Jamdesk (se encuentra en la configuración del dashboard)
- Tu dominio personalizado agregado a tu proyecto en el dashboard de Jamdesk (el Worker devuelve un error 403 hasta que el dominio esté registrado y verificado)
...
1. Comprobar que wrangler 3.0 o superior esté instalado
...
6. Desplegar en Cloudflare de forma opcional

### Selección de cuenta

La CLI verifica que hayas iniciado sesión en la cuenta de Cloudflare correcta antes de continuar. Si tienes acceso a varias cuentas de Cloudflare (algo común en agencias o equipos), se te pedirá que elijas una:
...
### Despliegue por CI o mediante scripts

Para CI o scripts, usa opciones para omitir las preguntas:

```bash
jamdesk deploy-proxy cloudflare --slug myproject --domain example.com --skip-deploy --yes
```

Con `--yes` la CLI no puede preguntar, así que necesita saber tu subdominio. Lo obtiene del proyecto vinculado en `docs.json`; si falta ese vínculo (ejecuta `jamdesk deploy` una vez para agregarlo), pasa `--slug` explícitamente, o el comando se detiene en lugar de adivinar.
...
- La configuración correcta a partir de los ajustes de tu dashboard

Sin este encabezado, las solicitudes se rechazarán con un error 403.
</Warning>
...
<Tip>
Si tu sitio también recibe tráfico en `www.yoursite.com`, agrega una segunda ruta para que el Worker gestione ambos casos:
...
Si prefieres la sintaxis anterior de Service Worker (compatible con Workers más antiguos):
...
Los Workers solo se ejecutan en registros DNS con el estado **Proxied** (nube naranja). Si tu registro A está configurado como "DNS only" (nube gris), las solicitudes van directo al origen y omiten el Worker por completo.

**Solución:** Cambia el registro A a Proxied (nube naranja) en el DNS de Cloudflare. Lo mismo aplica para subdominios: cualquier registro con una ruta de Worker debe estar en modo Proxied.
</Accordion>

<Accordion title="La verificación del dominio se queda en Pending">
Jamdesk verifica la propiedad leyendo directamente los valores de tu registro DNS. El proxy de Cloudflare (nube naranja) enmascara estos valores, por lo que la verificación no puede completarse.
...
Los recursos estáticos bajo `/_next/` y `/_jd/` (JavaScript, CSS, fuentes, imágenes) se sirven con encabezados de caché `immutable` de larga duración, por lo que Cloudflare los almacena en caché en el edge. Sus nombres de archivo tienen un hash de contenido, así que cada build produce nuevas URLs y los recursos actualizados se detectan automáticamente. No se requiere purga.

`cacheEverything: true` permite que Cloudflare almacene en caché esos recursos estáticos en la ruta con proxy; no anula el `no-store` del HTML. Para limpiar la caché del edge manualmente, usa **Purge Cache** de Cloudflare (Caching → Configuration → Purge Everything).
</Accordion>

<Accordion title="Versión de Wrangler demasiado antigua">
La CLI requiere wrangler 3.0 o superior. Actualiza con:
