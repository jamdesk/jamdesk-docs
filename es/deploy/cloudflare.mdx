...
Con `--yes` la CLI no puede mostrar preguntas, así que necesita conocer tu subdominio. Lo lee del proyecto vinculado en `docs.json`; si falta ese vínculo (ejecuta `jamdesk deploy` una vez para agregarlo), pasa `--slug` de forma explícita o el comando se detiene en lugar de adivinar.
