...
<AccordionGroup>
  ...
  <Accordion title="Se encontraron enlaces rotos">
    ...
    **Corrígelos automáticamente.** Si un enlace roto tiene un destino no ambiguo (ancla con errata o desfase de ancla entre idiomas), ejecuta `jamdesk fix --dry-run` para previsualizar los cambios y luego `jamdesk fix` para aplicarlos. Solo se reescriben los enlaces cuyo ancla corregida corresponde a un título real en la página de destino; los casos ambiguos quedan para que los corrijas a mano. Ver [Corregir enlaces rotos automáticamente](/es/cli/fix-broken-links).
  </Accordion>
