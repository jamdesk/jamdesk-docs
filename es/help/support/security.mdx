...
Cómo Jamdesk protege su documentación y los datos de su cuenta.
...
|---------------------|------------|
| En tránsito | TLS 1.3 entre usted, nuestro edge y nuestros orígenes |
...
Puede iniciar sesión en el dashboard con correo electrónico y contraseña, o mediante inicio de sesión único a través de Google o GitHub. Las contraseñas nunca se almacenan en texto plano; nuestro proveedor de autenticación conserva únicamente hashes unidireccionales.

Si inicia sesión con Google o GitHub, cualquier MFA que tenga configurado en esa cuenta (TOTP, llaves de seguridad, passkeys) se aplica también a su inicio de sesión en Jamdesk. Para mayor protección, recomendamos activar MFA en su cuenta de Google o GitHub y utilizar SSO.
...
Jamdesk se conecta a sus repositorios mediante una GitHub App con los permisos mínimos necesarios:

- Acceso de solo lectura al contenido del repositorio
- Entrega de Webhook para disparadores de build
- Sin acceso de escritura a su código

Nunca hacemos push, creamos ramas ni modificamos su repositorio. Puede revocar el acceso de Jamdesk en cualquier momento desde la configuración de [Applications](https://github.com/settings/installations) de GitHub.

### Tokens y Secretos

Cada secreto que nos confía se cifra en reposo. Esto incluye:

- Conexiones de la GitHub App, donde conservamos únicamente su ID de instalación numérico. Los tokens de instalación de corta duración (válidos por una hora) se generan bajo demanda y nunca se persisten.
...
Las API keys programáticas se almacenan como hashes unidireccionales. El texto en claro se muestra exactamente una vez en el momento de su creación. Si lo pierde, rote la clave.

## Gestión de Datos

### Qué Almacenamos

- Su configuración `docs.json` y cualquier asset de branding que haya subido
...
- Secretos cifrados para cualquier integración de terceros que haya configurado

### Qué No Almacenamos

- Su código fuente. Clonamos el repositorio en el momento del build y descartamos el árbol de trabajo cuando el contenedor de build se destruye.
- Credenciales de GitHub de larga duración. Consulte **Tokens y Secretos** más arriba.
...
Consulte nuestra [Política de Privacidad](https://jamdesk.com/privacy) para más detalles.

## Respuesta a Incidentes

Si un incidente de seguridad afecta a su cuenta o datos, tomamos tres medidas:
...
¿Ha encontrado un problema de seguridad? Repórtelo de forma responsable:

- Envíe un correo a `security@jamdesk.com`
- Incluya una descripción, los pasos para reproducirlo y una evaluación del impacto
...
<Columns cols={2}>
  <Card title="Contactar con Soporte" icon="headset" href="/es/help/support/contact">
    Contacte con el equipo de Jamdesk
  </Card>
