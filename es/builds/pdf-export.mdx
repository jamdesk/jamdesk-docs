...
description: Exporta todo tu sitio de documentación como un único PDF. Descargable desde el dashboard y enviado por correo. Solo planes de pago.
---

Exporta tu sitio de documentación como un único PDF desde el dashboard. Jamdesk renderiza todas las páginas publicadas en un solo archivo, te envía un enlace de descarga por correo cuando está listo y mantiene el PDF disponible para su nueva descarga hasta el próximo build de tu documentación.
...
<Card title="Descargar un PDF de muestra" icon="file-pdf" href="https://github.com/jamdesk/jamdesk-docs/releases/download/sample-pdf-v1/jamdesk-docs.pdf">
  Mira cómo es una exportación real. El sitio completo de documentación de Jamdesk, renderizado a través de este mismo proceso. 408 páginas, 6.6 MB.
</Card>

La exportación PDF es útil para:
...
- Capturas de cumplimiento y auditoría en un commit específico
- Paquetes de entrega a clientes

## Quién puede usarlo

La exportación PDF está disponible en todos los **planes de pago**. Sin complementos ni cargos adicionales. Cada proyecto puede generar hasta **3 PDF al día**, con el contador reiniciándose a medianoche UTC.

## Generar un PDF

<Steps>
  <Step title="Abre Configuración → Exportaciones PDF">
  ...
  <Step title="Haz clic en Generate PDF">
    El botón cambia a **Generating…** mientras Jamdesk renderiza el sitio en un navegador sin interfaz y escribe el PDF en el almacenamiento. El tiempo de finalización escala con el número de páginas y la cantidad de imágenes que contienen. La mayoría de los sitios terminan en pocos minutos.
  </Step>
  <Step title="Espera el correo">
    Cuando el PDF esté listo, enviamos al usuario con sesión iniciada un correo con un enlace de vuelta a la página de Settings. También puedes dejar la página abierta; el estado se actualiza en tiempo real.
  </Step>
  <Step title="Descargar">
    Haz clic en **Download PDF**. Consulta [Vigencia del enlace de descarga](#vigencia-del-enlace-de-descarga) si el enlace alguna vez expira.
  </Step>
</Steps>
...
<Steps>
  <Step title="Publica el nuevo contenido">
    Envía tus cambios a GitHub o usa `jamdesk deploy` desde el CLI.
  </Step>
  <Step title="Espera a que el build termine">
...
Es seguro hacer clic en Regenerate sin restricciones. Si no se ha completado ningún nuevo build, no volverá a renderizar ni a enviar correo.
...
El PDF incluye todas las páginas listadas en la navegación de tu `docs.json` para el idioma elegido, en el orden de declaración. Los enlaces externos en la navegación se omiten. Las páginas huérfanas (archivos en tu repositorio que no están conectados a la navegación) no se incluyen.
...
Cuando el PDF está listo, la persona que hizo clic en **Generate PDF** recibe un correo con un enlace de vuelta a la sección Settings → PDF Exports, además del número de páginas, la marca de tiempo y el idioma (para proyectos multilingüe). El dashboard también cambia al estado listo en tiempo real; no necesitas el correo para descargar.

Solo se envía correo al solicitante. Los demás colaboradores ven el nuevo PDF la próxima vez que abren la página de Settings.

<Warning>
  Si el correo no llega en unos minutos tras la finalización, revisa tu carpeta de spam. El PDF sigue disponible desde el dashboard independientemente de si el correo llega o no.
</Warning>

## Vigencia del enlace de descarga

Los enlaces de descarga por correo son válidos durante **72 horas** desde el momento en que el PDF finaliza. El dashboard genera una URL nueva cada vez que abres la página de Settings, por lo que las descargas desde el dashboard no caducan. Si un enlace de correo alguna vez devuelve un 403, ve al dashboard y haz clic en **Download PDF** desde allí.

## Solución de problemas

<Accordion title="'No completed build found — trigger a build first'">
  La exportación PDF renderiza un sitio ya construido, no tu MDX fuente. Necesitas al menos un build exitoso en tu rama predeterminada. Envía un commit o haz clic en **Rebuild** en la página de Builds, espera a que termine y vuelve a intentarlo.
</Accordion>
...
<Accordion title="Regenerate no produjo un archivo nuevo">
  El PDF en caché fue reutilizado porque no se ha completado ningún nuevo build desde la última exportación. Envía un commit o haz clic en **Rebuild** en la página de Builds, luego intenta Regenerate de nuevo una vez que el build termine. Consulta [Cache y Regenerar](#cache-y-regenerar).
</Accordion>

<Accordion title="El PDF tiene páginas que faltan y que sí existen en mi sitio">
...
<Columns cols={2}>
  <Card title="Activar Builds" icon="hammer" href="/es/builds/triggering">
    La exportación PDF se ejecuta contra el build completado más reciente. Lánzalo manualmente o al hacer push.
  </Card>
  <Card title="Optimización de imágenes" icon="image" href="/es/builds/image-optimization">
    Convierte PNG/JPG a WebP en el momento del build para mantener los PDF pequeños.
  </Card>
</Columns>
