...
`<EmailSubscribe>` coloca un formulario de suscripción por correo dentro de una página de documentación con una sola etiqueta MDX. Cuando has [conectado un proveedor](/es/integrations/email-signups) en el dashboard, muestra un formulario alojado por Jamdesk que añade nuevos suscriptores directamente a tu audiencia. Sin un proveedor conectado, puede alojar en su lugar la incrustación para pegar de otro servicio. Está pensado para páginas de registro de cambios y notas de versión, donde los lectores quieren enterarse de las novedades.
...
- **Proveedores nativos** (`resend`, `mailchimp`, `kit`, `loops`, `beehiiv`, `brevo`, `sendgrid`) muestran un formulario alojado por Jamdesk. Jamdesk captura la dirección y la añade mediante tu clave conectada. Esta es la vía que necesita un proveedor [conectado en el dashboard](/es/integrations/email-signups).
