...
Tanto `heading` como `body` aceptan los mismos campos. Consulta [Tematización → Tipografía](/es/customization/theming#tipograf-a) para orientación sobre la elección de fuentes.
...
Si tu sitio es multilingüe, coloca un archivo `<spec>.<lang>.<ext>` junto a cada especificación fuente (p. ej., `openapi/api.fr.yaml`) y Jamdesk lo sirve en las URLs de ese idioma. Consulta [Traducción de especificaciones OpenAPI](/es/setup/languages#traducci-n-de-especificaciones-openapi).
...
Activa la protección con contraseña compartida para tu sitio. Solo configuración declarativa. La frase de contraseña real la estableces en el [dashboard](/es/setup/password-protection#rotaci-n-y-revocaci-n-de-sesiones) después de que se ejecute el próximo build.
