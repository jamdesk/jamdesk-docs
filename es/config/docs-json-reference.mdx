...
| Campo | Tipo | Predeterminado | Descripción |
|-------|------|---------|-------------|
| `autoRedirect` | boolean | `false` | Cuando es `true`, un visitante que llega por primera vez a la raíz de tu documentación se envía al idioma que coincide con la cabecera `Accept-Language` de su navegador. Requiere dos o más entradas en `navigation.languages`. |

Solo se redirigen las raíces. Un enlace profundo como `/guides/authentication` siempre sirve la página que nombra, así que un enlace que pegues en un ticket abre la misma página para todo el mundo. Los rastreadores de los motores de búsqueda nunca se redirigen, así que tus etiquetas `hreflang` siguen decidiendo qué se indexa.

El idioma del visitante se recuerda durante un año. Elegir un idioma en el selector anula la coincidencia automática a partir de ese momento.

Consulta [Soporte Multilingüe → Enrutamiento automático de idioma](/es/setup/languages#enrutamiento-automático-de-idioma) para conocer el comportamiento completo.
