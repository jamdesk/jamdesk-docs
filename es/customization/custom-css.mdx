...
description: "Añade CSS para sobreescribir estilos del tema, ajustar tipografía o añadir estilos de marca. Jamdesk expone variables CSS para ajustar lo más habitual."
---
...
Crea un archivo `style.css` en la raíz de tu proyecto — la misma carpeta que tu `docs.json`. Jamdesk lo aplica automáticamente a todas las páginas; no necesitas ninguna entrada en `docs.json`.

Cualquier archivo `.css` en la raíz del proyecto funciona, no solo `style.css` — útil al migrar desde otra herramienta. Cuando hay varios, Jamdesk los combina en orden alfabético de nombre de archivo.
...
- En la vista previa local (`jamdesk dev`), los cambios en `style.css` se aplican al recargar el navegador; en tu sitio publicado, surten efecto en la siguiente compilación
