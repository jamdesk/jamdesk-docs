...
description: Agrega CSS personalizado para sobrescribir estilos del tema, ajustar tipografía o aplicar tu marca. Jamdesk expone variables CSS para los ajustes más comunes.
---

Los temas de Jamdesk incluyen valores predeterminados sensatos, pero puedes agregar CSS personalizado para adaptar tu marca o ajustar estilos específicos.
...
Cualquier archivo `.css` en la raíz del proyecto funciona, no solo `style.css` — útil al migrar desde otra herramienta. Si hay varios, Jamdesk los combina en orden alfabético por nombre de archivo.

Ejecuta `jamdesk dev` para previsualizar el resultado localmente — renderiza tu CSS personalizado de la misma manera que el sitio publicado.
...
Los colores primarios se configuran en `docs.json`. Para un control más detallado, sobreescribe estas variables:
...
O aloja las fuentes localmente:
...
- En la preview local (`jamdesk dev`), los cambios en tus archivos CSS se aplican al actualizar el navegador; en el sitio publicado, los cambios surten efecto en el próximo build
