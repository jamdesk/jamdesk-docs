...
<Steps>
  ...
  <Step title="Generar la clave de firma">
    ...
    Jamdesk crea un par de claves Ed25519, conserva únicamente la clave *pública* y muestra la clave *privada* exactamente una vez. Cópiala inmediatamente en tu gestor de secretos. Jamdesk nunca almacena ni envía por correo electrónico la clave privada, y no puede recuperarla si la pierdes. Si ocurre, rota la clave. La rotación es un cambio inmediato y sin periodo de solapamiento, así que lee [Rotar la clave de firma](#rotar-la-clave-de-firma) antes de hacer clic.
  </Step>
...
Marcar una página como pública abre la página en sí. Las imágenes y los vídeos que contiene se sirven desde las rutas de recursos de tu proyecto, que siguen detrás de la barrera, así que un visitante sin sesión ve una página pública sin sus ilustraciones. Añade esas rutas a `auth.jwt.public[]` cuando una página pública las necesite:
...
Limita el glob a las carpetas que tus páginas públicas usan de verdad. Las rutas de recursos nunca se comprueban contra los grupos, así que un glob amplio como `/_jd/images/**` sirve todas las imágenes del sitio a cualquier persona, incluidas las capturas de pantalla de las páginas que restringiste con `groups`. Guarda las imágenes de las páginas públicas en su propia carpeta y abre solo esa carpeta.
...
- Jamdesk deduce qué carpetas de primer nivel son traducciones a partir de `navigation.languages`, más cualquier carpeta de primer nivel que lleve el nombre de un código de idioma (`fr`, `it`, `cs`, etc.) y contenga páginas. Una carpeta que solo comparte el nombre con un código de idioma, por ejemplo una carpeta `it` que contiene los manuales de operaciones del departamento de TI y no traducciones al italiano, también se trata como una traducción, y sus páginas heredan los `groups` de cualquier página raíz que esté en la misma ruta. Esto solo puede restringir más, nunca menos. Cambia el nombre de la carpeta si eso te causa problemas.
- `groups` restringe páginas, no las imágenes, los vídeos ni los demás archivos que una página incrusta. Un recurso al que solo enlaza una página restringida se sigue sirviendo a cualquier visitante con sesión iniciada que pida su URL, sean cuales sean los grupos que lleve su sesión. Las URL de recursos siguen la estructura de archivos de tu repositorio, así que un nombre como `images/admin/sso-config.png` es fácil de adivinar. Mantén fuera del repositorio de documentación todo lo que no quieras mostrar a todos tus lectores con sesión iniciada.
- La barra lateral, las pestañas, las rutas de navegación y los enlaces de página anterior y siguiente se filtran para cada visitante. Una página que los grupos del visitante no cubren queda fuera, y un grupo o una pestaña que se quede vacío desaparece con ella, de modo que el nombre de una sección restringida no se muestra a quienes no pertenecen a ella. Este filtrado ocurre en el momento de la solicitud y es independiente de las exclusiones durante el build descritas arriba, que se aplican a todo el mundo.
...
| `expiresAt` | No | Marca de tiempo Unix (segundos) que indica cuándo debe expirar la *sesión* resultante. Está limitada a 30 días; si se omite, el valor predeterminado es de 7 días. Es independiente de la declaración `exp`, de corta duración, del propio token. |
...
**Rotate signing key**, en la tarjeta del dashboard, genera un nuevo par de claves y muestra la nueva clave privada una sola vez, igual que hizo la primera generación. No hay periodo de solapamiento. En unos 15 segundos la clave antigua deja de aceptarse y todas las sesiones existentes terminan. Hasta que tu backend firme con la clave nueva, se rechaza cada inicio de sesión y los visitantes rebotan entre tu página de inicio de sesión y la documentación.

Por eso el orden importa:

1. Ten preparado un despliegue que lea la clave de firma desde tu gestor de secretos, en lugar de un valor escrito directamente en el código.
2. Haz clic en **Rotate signing key** y copia la nueva clave privada.
3. Actualiza el secreto y despliega. Los inicios de sesión vuelven a funcionar en cuanto tu backend usa la clave nueva.

Rota en un momento de poca actividad si puedes, y avisa a quien se encargue del despliegue del backend antes de hacer clic.

Si lo único que quieres es terminar la sesión de todo el mundo, por ejemplo después de que se pierda un portátil, usa **Revoke sessions**. Conserva la clave, así que no cambia nada en tu backend; cada visitante solo tiene que volver a iniciar sesión.

**Clear signing key** elimina la clave pública de Jamdesk. `auth.jwt` sigue habilitado en `docs.json`, por lo que el sitio permanece protegido, pero no se puede verificar ningún token hasta que generes una clave nueva. Bórrala solo cuando vayas a pasar el sitio a otro modo de acceso o a retirarlo.
...
Cerrar sesión en la documentación no cierra la sesión del visitante en tu producto. Si tu flujo de inicio de sesión firma un token para cualquiera que ya tenga una sesión abierta en la aplicación, un visitante que haga clic en **Log out** y después abra un enlace a la documentación vuelve a entrar de inmediato. Normalmente eso es lo que quieres. Si necesitas un cierre de sesión real, haz que el controlador de tu `loginUrl` compruebe que hay un inicio de sesión explícito en lugar de emitir un token automáticamente, o apunta también tu propio cierre de sesión a la URL de cierre de sesión de la documentación.
...
<Accordion title="El dashboard muestra un banner «Runtime out of sync»">
  El dashboard y la caché del runtime no coinciden sobre tu clave de firma, normalmente porque un error temporal de escritura interrumpió una generación, una rotación o un borrado. El banner indica en qué sentido: o bien la última clave todavía no ha llegado a la caché (los tokens firmados con ella pueden rechazarse), o bien una clave que borraste sigue almacenada en la caché (los tokens firmados con ella se siguen aceptando). Jamdesk lo vuelve a comprobar cada vez que abres la página de configuración. Si el banner permanece, haz clic en **Retry sync**. Si eso sigue fallando, rota la clave; en el caso de la clave borrada, genera una y vuelve a borrarla. Un banner que diga que Jamdesk no ha podido verificar el estado significa que la comprobación en sí falló; reinténtalo cuando el runtime vuelva a estar accesible.
</Accordion>
