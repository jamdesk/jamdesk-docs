...
Tu endpoint MCP es la URL de tu documentación con `/_mcp` añadido. Si tienes un [dominio personalizado](/es/deploy/custom-domains) activo, úsalo:
...
<Note>
  Prefiere el endpoint del dominio personalizado siempre que haya uno conectado — tu marca aparece en cada URL que ven las herramientas de IA, y el endpoint sigue funcionando aunque más adelante ocultes tu subdominio `.jamdesk.app` con el modo «solo dominio personalizado».
</Note>
