...
<Tip>
  Agrega la terminología de tu producto, las convenciones de nomenclatura de la API y cualquier regla de estilo específica de tu documentación.
</Tip>
...
Reemplaza `your-project` con tu subdominio de Jamdesk. Consulta [Servidor MCP](/es/ai/mcp-server) para conocer los detalles del endpoint y la configuración de dominio personalizado.

## Prompts de ejemplo

Codex ejecuta tareas de forma asíncrona. Eso cambia la forma en que le das instrucciones: en lugar de un ida y vuelta interactivo, lanzas una tarea larga y revisas el resultado más tarde, así que los prompts que mejor funcionan son los que piden un bloque de trabajo completo.

Un buen primer intento: *"Escribe la documentación de la API de autenticación basándote en el código fuente de /src/auth"*. Codex lee tu base de código, encuentra los archivos relevantes y genera las páginas correspondientes. Para algo más complicado, apúntalo hacia tu historial real de errores: *"Crea una página de solución de problemas que cubra los 5 errores más comunes en nuestros issues de GitHub"* tiende a producir una página más honesta que cualquier cosa que escribirías de memoria.

Las reestructuraciones multiarchivo funcionan especialmente bien en este modo. Dale a Codex un README de 500 líneas y pídele que divida el archivo en páginas de documentación separadas con la navegación adecuada; volverá con un árbol que puedes revisar en un solo PR.

## La skill /update-jamdesk

Para actualizaciones automatizadas de la documentación cuando cambia el código, instala la skill `/update-jamdesk`:
...
<Columns cols={2}>
  <Card title="Escribir con IA" icon="wand-magic-sparkles" href="/es/ai/writing-with-ai">
    Estrategias de prompts que funcionan en todas las herramientas
  </Card>
  <Card title="Claude Code" icon="terminal" href="/es/ai/claude-code">
    Plantilla de CLAUDE.md y flujos de trabajo con contexto de todo el proyecto
  </Card>
</Columns>
