...
description: >-
  ...
  documentación por lotes, multiarchivo y asíncronas contra repos de GitHub.
---

[Codex](https://openai.com/index/introducing-codex/) es el agente de programación en la nube de OpenAI. Funciona directamente con repos de GitHub, así que puedes ejecutar trabajo de documentación contra tu proyecto sin configurar antes un entorno local.

La diferencia entre Codex y [Claude Code](/es/ai/claude-code) tiene que ver sobre todo con la forma del flujo de trabajo. Codex se ejecuta en la nube y trabaja de forma asíncrona: describes un bloque de trabajo, te alejas y revisas un PR cuando está listo. Eso encaja bien con tareas por lotes (dividir un README de 500 líneas en una docena de páginas, o generar una página de solución de problemas a partir de tus últimos seis meses de issues de GitHub), pero no es ideal para el ida y vuelta de refinar una sola página. Si tu trabajo es una tarea por lotes multiarchivo que prefieres no supervisar de cerca, Codex es la opción correcta. Para trabajo interactivo en una sola página, usa [Claude Code](/es/ai/claude-code) en su lugar.
...
<Steps>
  ...
  <Step title="Agrega instrucciones para el agente">
  ...
  <Step title="Conecta el servidor MCP">
    Agrega el endpoint MCP de tu documentación a `.codex/config.toml` para que Codex pueda buscar en tu documentación publicada.
  </Step>
</Steps>
...
<Tip>
  Agrega tu terminología de producto, tus convenciones de nomenclatura de API y cualquier regla de estilo específica de tu documentación.
</Tip>

## Configuración de MCP

Agrega el endpoint de tu documentación a `.codex/config.toml`:
...
Reemplaza `your-project` con tu subdominio de Jamdesk — o, si tienes un dominio personalizado activo, úsalo en su lugar: `url = "https://docs.acme.com/_mcp"`. Consulta [MCP Server](/es/ai/mcp-server) para conocer los detalles del endpoint.

## Ejemplos de prompts

Codex ejecuta tareas de forma asíncrona. Eso cambia cómo lo prompteas: en lugar de un ida y vuelta interactivo, lanzas una tarea larga y revisas después, así que los prompts que valen la pena son los que piden un bloque de trabajo.

Un buen primer intento: *"Escribe la documentación de la API de autenticación basándote en el código fuente de /src/auth"*. Codex lee tu código, encuentra los archivos relevantes y genera páginas coherentes. Para algo más desordenado, apúntalo a tu historial real de bugs: *"Crea una página de solución de problemas que cubra los 5 errores más comunes en nuestros issues de GitHub"* suele producir una página más honesta que cualquier cosa que escribirías de memoria.

Las reestructuraciones multiarchivo funcionan especialmente bien en este modo. Entrégale a Codex un README de 500 líneas y pídele que divida el archivo en páginas de documentación separadas con la navegación correspondiente; volverá con un árbol que puedes revisar en un solo PR.
...
Consulta [Automated Updates](/es/ai/automated-updates) para la guía completa.
