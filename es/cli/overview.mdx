...
<Accordion title="jamdesk deploy-proxy cloudflare" icon="cloud">
  Genera y despliega un Cloudflare Worker que enruta `/docs` de tu propio dominio hacia tu sitio de Jamdesk.

  ```bash
  jamdesk deploy-proxy cloudflare
  jamdesk deploy-proxy cloudflare --slug miproyecto --domain example.com --skip-deploy --yes
  ```

  Interactivo por defecto: verifica Wrangler, valida tu cuenta de Cloudflare, detecta automáticamente tu slug desde `docs.json`, genera los archivos del Worker y despliega opcionalmente.

  | Opción | Descripción |
  |--------|-------------|
  | `--slug <slug>` | Slug del proyecto Jamdesk |
  | `--domain <domain>` | Dominio objetivo (p. ej. `example.com`) |
  ...
  | `--force` | Sobrescribir el directorio existente sin confirmación |
  | `--yes` | Omitir todas las confirmaciones (modo CI) |

  <Card title="Guía de Cloudflare Workers" icon="cloud" href="/es/deploy/cloudflare">
    Configuración del Worker, patrones de ruta y gestión de caché
  </Card>
</Accordion>
