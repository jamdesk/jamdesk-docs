...
<Accordion title="jamdesk deploy-proxy cloudflare" icon="cloud">
  ...
  Interactivo por defecto: comprueba Wrangler, verifica tu cuenta de Cloudflare, detecta automáticamente tu slug desde `docs.json`, genera los archivos del Worker y, opcionalmente, despliega. Con `--yes` genera los archivos y se detiene — despliega con `npx wrangler deploy` desde el directorio de salida.
  ...
  | `--skip-deploy` | Omitir la pregunta «¿desplegar ahora?» en una ejecución interactiva |
  | `--force` | Sobrescribir el directorio de salida si ya existe |
  | `--yes` | Responder cada pregunta con su valor por defecto (modo CI). Nunca despliega ni sobrescribe un directorio existente |
