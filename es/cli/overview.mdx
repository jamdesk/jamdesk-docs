...
La CLI de Jamdesk te permite previsualizar docs localmente, validar la configuración, comprobar enlaces rotos y migrar desde otras plataformas. Es de código abierto bajo la [Apache License 2.0](https://github.com/jamdesk/jamdesk-cli).
...
<Note>
  **Tus specs OpenAPI se validan en tres lugares.** `jamdesk dev` se detiene al inicio si una spec referenciada es inválida, y `jamdesk validate` / `jamdesk openapi-check` revisan las specs cuando lo pidas. Al desplegar, el build en la nube también valida tus specs referenciadas — pero ahí es una **advertencia no bloqueante**: el resto de tu documentación se publica igualmente y se te indica exactamente qué falla (un error de análisis con línea y columna, un `$ref` sin resolver o un `operationId` duplicado) por correo y en la lista de builds del dashboard. Corrige la spec y vuelve a desplegar para que desaparezca.
</Note>
