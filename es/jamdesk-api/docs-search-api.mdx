...
description: Busca tu documentación de Jamdesk programáticamente. Potencia chatbots, bots de Slack, búsqueda personalizada y agentes de IA con respuestas al día.
---
...
Si su sitio de documentación admite varios idiomas, la API filtra los resultados a un solo idioma por solicitud. Indique `language` en el cuerpo de la solicitud con un código BCP-47 (por ejemplo `en`, `es`, `fr`, `pt-BR`, `zh-Hans`).
...
| Regla | Detalle |
|-------|---------|
| **Por defecto** | `en` (inglés). Omita el campo, o pase `null`, para usar el valor predeterminado. |
...
| **Códigos de 3 segmentos** | No compatibles actualmente. Códigos como `zh-Hant-HK` o `sr-Latn-RS` devuelven `400`. [Contacte con soporte](mailto:support@jamdesk.com) si los necesita. |
...
| **Proyectos monolingües** | El filtro se ignora; siempre obtiene el conjunto completo de resultados. Enviar `language` no es un error, es un no-op. |
| **Devuelto en la respuesta** | Cada respuesta 200 incluye un campo `language` con el valor resuelto por el servidor (valor de la solicitud, o `en` por defecto). |

<Info>
Un proyecto es multilingüe cuando su `docs.json` tiene un arreglo `navigation.languages` con dos o más entradas. Para comprobarlo, abra la pestaña **Settings → Languages** del panel de control, o revise `docs.json` directamente.
</Info>

<Warning>
La opción predeterminada «inglés» se aplica incluso si su proyecto no tiene versión en inglés. Si su proyecto multilingüe es, por ejemplo, solo francés y español, llamar al endpoint sin `language` filtrará por `en` y devolverá un resultado vacío. Pase siempre un `language` explícito desde sitios sin versión en inglés.
</Warning>
