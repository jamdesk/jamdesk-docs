...
| **Proyectos multilingüe** | El filtro es estricto: solo se devuelven fragmentos etiquetados con el idioma solicitado. Una solicitud para `de` en un proyecto que solo tiene inglés y francés devuelve un conjunto de resultados vacío, no un `400`. |
...
Actualmente no hay SDKs oficiales para ningún lenguaje. Usa la REST API directamente mediante `fetch`, `requests`, `curl` o cualquier cliente HTTP. La [colección de Postman](#colecci-n-de-postman) que se muestra a continuación ofrece ejemplos listos para clonar.
