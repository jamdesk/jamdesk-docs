...
### Corrección automática de enlaces rotos

Cuando un enlace roto tiene un destino no ambiguo — un ancla con errata o desfase de ancla entre idiomas donde una página traducida renombró sus títulos — `jamdesk fix` lo repara automáticamente:

```bash
jamdesk fix --dry-run   # previsualizar cada corrección planificada, sin escribir nada
jamdesk fix             # aplicar tras una confirmación y/N
```

Solo reescribe un enlace cuando el ancla corregida corresponde a un título real en la página de destino, dejando cualquier caso ambiguo para revisión manual. Ver [Corregir enlaces rotos automáticamente](/es/cli/fix-broken-links).
