...
<Info>
The MCP server exposes the same search index as the Docs Search API. Use the Docs Search API (with a `jdk_` key) when you need authenticated access or want to control rate limits per integration.
</Info>
...
<Warning>
The `/_api` route requires authentication via your `jdk_` API key regardless of how the request is routed. Proxying through Cloudflare does not bypass authentication.
</Warning>
