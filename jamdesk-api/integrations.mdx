...
## Subpath Hosting (Cloudflare Worker)

If you're using [subpath hosting](/deploy/subpath-hosting) to serve your docs at a path on your domain — `/docs` by default, or a custom subpath like `/help` — the `/_api` route needs to be proxied through your Cloudflare Worker too, alongside your docs path.

Add `/_api` to the `PROXY_PATHS` array in your worker, using whichever path prefix you configured in the dashboard:
...
This ensures search requests to `yoursite.com/_api/search` (or `yoursite.com/help/_api/search`, if you're on a custom subpath) are forwarded to your Jamdesk docs site.
