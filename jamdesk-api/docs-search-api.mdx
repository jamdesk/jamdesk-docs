...
| **401** | `invalid_key_format` | `Authorization` header is missing or the key doesn't match `jd_live_<32 hex>` | Check the header format — must be `Bearer jd_live_...` |
...
<Info>
401 and 403 are permanent failures — retrying with the same key won't help. 429, 502, and 503 are transient — retry with exponential backoff.
</Info>
