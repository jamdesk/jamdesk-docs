...
Jamdesk builds documentation sites from MDX files in your GitHub repo. You write Markdown with React components, push to GitHub, and your site is live on a global CDN in under 60 seconds. Your content stays in your repo as plain MDX files, so you can move it to another tool at any time.
...
Every site includes these features by default. No configuration is required.
...
<Columns cols={2}>
  ...
  <Card title="Built-in Analytics" icon="chart-bar" href="/setup/analytics-overview">
    Privacy-focused analytics dashboard with no cookies required. Or bring your own: Jamdesk supports Google Analytics, Plausible, PostHog, and more.
  </Card>
</Columns>
...
<Columns cols={2}>
  <Card title="Minimal Permissions" icon="shield-check">
    The GitHub App only requests read access to repositories you select. The app has no write access and requests no organization-wide scopes.
  </Card>
  ...
  <Card title="HTTPS Everywhere" icon="lock">
    Automatic SSL certificates for custom domains and subdomains. Certificates are issued and renewed automatically.
  </Card>
...
Every Jamdesk site ships from the edge with sub-50ms time to first byte. That is fast enough for Google's ranking thresholds and for AI crawlers to fetch and cite.
