...
description: Learn how Jamdesk builds and deploys your documentation—from GitHub push to global CDN in under 60 seconds
---

Jamdesk automates the entire documentation lifecycle. Push your MDX files to GitHub, and Jamdesk handles building, optimizing, and deploying to a global CDN—typically in under 60 seconds.
...
|---------|--------------|----------|
| **Git Push** | Webhook fires on push to connected branch | Normal development workflow |
| **Dashboard** | Click "Rebuild" in project settings | Force refresh after config changes |
| **API** | `POST /projects/{project_id}/rebuild` with API key | CI/CD pipelines, automated workflows |

<Tip>
Builds are debounced—if you push multiple commits quickly, Jamdesk waits for activity to settle before building.
</Tip>

For API details and authentication, see [Builds API / Triggers](/reference/builds-api).
...
<Columns cols={2}>
  <Card title="Create Your First Project" icon="plus" href="/setup/creating-projects">
