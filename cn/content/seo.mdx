...
`robots` 元数据和 `noindex` 控制**搜索引擎**：页面是否出现在 Google 和你的 `sitemap.xml` 中。它们不会影响 AI 工具读取的 `llms.txt` 和 `llms-full.txt` 文件。若要停止发布这些文件，请将 `seo.ai.llmsTxt` 设置为 `false`（参见[关闭 llms.txt](/cn/ai/llms-txt#关闭-llmstxt)）。两种控制彼此独立：页面可以被搜索引擎索引但排除在 AI 摄取之外，反之亦然。
