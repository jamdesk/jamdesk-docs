...
| 字段 | 类型 | 默认值 | 描述 |
|-------|------|---------|-------------|
| `autoRedirect` | boolean | `false` | 为 `true` 时，首次到访文档根路径的访客会被送到与其浏览器 `Accept-Language` 头部匹配的语言。需要在 `navigation.languages` 中配置两个或更多条目。 |
...
完整行为请参阅[多语言支持 → 自动语言路由](/cn/setup/languages#自动语言路由)。
