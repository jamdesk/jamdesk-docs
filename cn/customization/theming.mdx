---
title: 主题
description: 选择内置主题控制字体、布局、间距和深色模式，并用品牌色覆盖默认配色。
---
...
<Tabs>
  ...
  <Tab title="Nebula" icon="cloud">
    ...
    页眉徽标布局，整体使用 IBM Plex Mono。浅色模式下采用暖奶油色背景（`#FEFCF9`）。最小化圆角（2-8px）。
  ...
  <Tab title="Pulsar" icon="bolt">
    ...
    侧边栏优先布局，徽标位于侧边栏中，导航使用左边框指示器。内容区域为全宽，并居中显示，宽度为 780px。默认隐藏页眉。
  ...
  <Tab title="Halo" icon="sun">
    ...
    页眉徽标布局，整体使用 Figtree。暖沙色背景（`#F7F2E8`），内容置于卡片上方。大圆角（8-28px）。
...
`weight` 仅设置**正文**文字的字重；标题会保留每种主题更重的字重，因此文字层级仍然清晰。它也适用于 `body` 形式：`"body": { "family": "Geist", "weight": 200 }`。

如果需要进行超出 `fonts` 字段范围的设置（覆盖代码字体、调整各级标题的字重，或加载非 Google 网页字体），请参阅[自定义 CSS](/cn/customization/custom-css#外部字体)。
...
<Tip>
使用 [Coolors](https://coolors.co) 或 [Realtime Colors](https://realtimecolors.com) 等配色工具生成协调的配色方案。
</Tip>

<Tip>
**需要的不只是颜色？**[自定义 CSS](/cn/customization/custom-css) 可用于覆盖字体、间距、内容宽度、边框圆角、提示框样式和组件级细节。主题设置默认值；对于 CSS 能够调整的其他内容，请使用自定义 CSS。
</Tip>
...
如需精细控制深色模式样式，请使用[自定义 CSS](/cn/customization/custom-css)。

## 下一步

<Columns cols={3}>
  <Card title="品牌设置" icon="palette" href="/cn/customization/branding">
    自定义徽标、颜色和 favicon
  </Card>
