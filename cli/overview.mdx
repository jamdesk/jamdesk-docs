...
<Accordion title="jamdesk migrate" icon="right-left">
  Migrate documentation from Mintlify to Jamdesk, or extract React hook components from existing Jamdesk projects.

  ```bash
  jamdesk migrate
  ```

  **For Mintlify projects**, the interactive wizard will:
  ...
  6. Extract React hook components to `/snippets` automatically

  **For existing Jamdesk projects**, the command scans for inline components using React hooks and extracts them to snippet files.

  **Automatic Hook Extraction**

  Inline MDX components that use React hooks (`useState`, `useEffect`, etc.) are automatically extracted to snippet files. This is required because inline components are server-rendered and cannot use client-side hooks.

  For example, this inline component:
  ...
  Becomes a snippet file at `/snippets/counter.tsx` with `'use client'` directive, and the MDX file gets an import statement instead.
  ...
  **Supported conversions:**
  ...
  | Inline components with hooks | Extracted to `/snippets` |
</Accordion>
