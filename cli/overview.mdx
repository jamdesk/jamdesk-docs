...
<Accordion title="jamdesk spellcheck" icon="spell-check">
  ...
  **Example output:**
  ...
  Uses an English dictionary with 150+ built-in tech terms (API, GraphQL, Kubernetes, React, etc.) so common jargon doesn't flag. Skips code blocks, inline code, frontmatter, JSX, URLs, and file paths.
  ...
  **Interactive fix mode (`--fix`)** steps through each unique misspelled word:
  ...
  ❯ Fix → receive
    Ignore (add to docs.json)
    Skip
  ```

  - **Fix** replaces the word with a suggestion across all files (prose-safe — won't modify code blocks or JSX attributes)
  - **Ignore** adds the word to `spellcheck.ignore` in your docs.json
  - **Skip** does nothing
