...
Check each planned fix. If a change doesn't look right, you can address it manually and exclude it from the run using `--types` to limit which fix categories apply.
...
Remaining warnings are ones where the target was ambiguous — `jamdesk fix` leaves those for you to resolve.
...
| `--types <list>` | Limit to specific fix types: `anchor-typo`, `locale-drift` |
