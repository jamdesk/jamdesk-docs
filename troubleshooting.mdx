...
<Tip>
The build log in your dashboard shows the exact file and line where the error occurred. Start there. It almost always points at the real problem, not just the symptom.
</Tip>
...
## Valid OpenAPI Spec Fails to Validate

`jamdesk dev` rejects a spec you know is valid, with errors like `#/servers/0/variables/host must NOT have unevaluated properties` — usually on server variables that carry a `description`, or on a license with only a `name`. The spec is fine; the CLI's copy of the OpenAPI 3.1 meta-schema is the problem. npm 12 blocks package install scripts by default, which skipped the step that corrects two known defects in that schema.

Upgrade the CLI — version 1.1.167 and later repair the schema at validation time, so the install step no longer matters:

```bash
npm install -g jamdesk@latest
```

If you're pinned to an older version, `npm install -g --allow-scripts=jamdesk jamdesk` lets the install step run instead.
...
A few common reasons your dashboard shows zero visitors. Analytics take up to 24 hours to appear after a site's first deploy, so brand-new projects look empty for a while. Ad blockers and Do Not Track stop a chunk of visits from being counted, so your numbers will always trail your server logs. If neither of those applies, confirm your site is actually deployed and publicly accessible.
