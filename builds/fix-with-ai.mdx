...
<Steps>
  ...
  <Step title="Read each proposed change">
    Fixes are grouped by type. **Broken links** shows each rewritten anchor, **Missing descriptions** shows the suggested summary for each page, and **Missing titles** shows the proposed title. Clear the checkbox on anything you would rather skip this time, or use the ignore button to [silence a recommendation for good](#ignore-recommendations). Issues with no clear fix appear under **Can't be fixed automatically** and are left for you to handle by hand.
...
Every row in the modal, fixable or not, has an ignore button. Click it and the item moves to a collapsed **Ignored** section at the bottom, and the banner count on the Project Home page drops to match after you close the modal. **Ignore all** does the same for everything still listed.
...
- **Restoring.** Expand the **Ignored** section and click **Restore** on any item. If you ignore everything on a build, the banner becomes a short note that the recommendations are ignored, with a **Review** button that reopens the modal.
