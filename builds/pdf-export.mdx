...
PDF export is on every **paid plan** — there is no add-on or extra charge. Free trial projects see the button but can't run an export until they upgrade.

## Generate a PDF

<Steps>
  <Step title="Open your project's Settings page">
    In the dashboard, select the project from the sidebar and click **Settings**.
  </Step>
  <Step title="Scroll to PDF Exports">
    The section lives beneath project metadata.
  </Step>
  ...
  <Step title="Download">
    Click **Download PDF**. The link is a short-lived signed URL; if it expires, refresh the Settings page to mint a new one.
  </Step>
</Steps>
...
<Tip>
  The default language exports at your site's root URLs (e.g. `docs.acme.com/getting-started`). Other locales export at their prefixed URLs (e.g. `/es/getting-started`).
</Tip>

## Caching and Regenerate

The Settings page remembers only the **most recent** completed export per project. When you click **Regenerate**, Jamdesk checks whether the last export matches both the current commit *and* the locale you're asking for:

- **Same commit and locale** → the cached PDF is handed back immediately. You'll see a notice explaining that nothing has changed since your last export.
- **Different commit or locale** → a fresh render kicks off.

Because the cache is a single slot, exporting a second locale or a new commit replaces whatever was there before. The earlier PDF isn't deleted, but it's no longer the one surfaced on the Settings page.
...
<Steps>
  ...
  <Step title="Regenerate">
    Back on the Settings page, click **Regenerate**. The new commit's SHA differs from the cached one, so Jamdesk kicks off a fresh render.
  </Step>
</Steps>

Regenerate is safe to click liberally — if the commit hasn't moved, it won't re-render or re-email.

## Rate limiting

You can produce one completed export per project every **five minutes**. Requests inside that window return `Try again in a few minutes`. The cache-hit path bypasses the rate limit — repeat downloads of an unchanged PDF never hit the wall.
...
If your site is [password protected](/setup/password-protection), the PDF exporter can't unlock it — the headless browser doing the rendering doesn't carry a session. Protected pages won't appear in the PDF and count against the export's failure budget. If most of your site is gated, disable password protection before running the export, or rely on unprotected pages only.
...
<Accordion title="Regenerate didn't produce a new file">
  The cached PDF was reused because your last completed build's commit hasn't changed. See [Caching and Regenerate](#caching-and-regenerate) — you need a new build to force a fresh render.
</Accordion>
...
<Columns cols={2}>
  <Card title="Triggering Builds" icon="hammer" href="/builds/triggering">
    PDF export runs against the most recent completed build. Here's how to kick one off.
  </Card>
