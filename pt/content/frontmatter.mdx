---
title: Frontmatter
description: Configure títulos, descrições, ícones, barras laterais e metadados de SEO usando o bloco YAML de frontmatter no topo de cada arquivo MDX.
---
...
| Campo | Tipo | Descrição |
|-------|------|-------------|
| `description` | string | Resumo breve para SEO e resultados de busca (50-160 caracteres) |
...
| `mode` | string | - | Defina como `"wide"` para usar um layout de largura total |
| `hideFooter` | boolean | `false` | Oculta o rodapé social nesta página |
| `rss` | boolean | `false` | Ativa a geração de feed RSS a partir dos componentes [Update](/pt/components/update) nesta página |
| `search` | boolean | `true` | Defina como `false` para excluir a página da busca do site, das respostas do chat de IA e do MCP. Ela permanece na barra lateral, no sitemap e no llms.txt. Consulte [Excluir uma página apenas da busca](/pt/content/hidden-pages#manter-uma-página-fora-da-busca-בלבד) |
...
| `public` | boolean | `false` | Isenta esta página da proteção por senha (usado quando todo o site é protegido por `auth.password.enabled`). Tem precedência sobre `private: true` quando ambos são definidos. |
...
| `seo` | object | - | Bloco aninhado que contém qualquer um dos campos acima, além de tags meta personalizadas arbitrárias |
