...
| `expiresAt` | Não | Timestamp Unix (em segundos) que define por quanto tempo a *sessão* resultante deve durar. Limitado a 30 dias; o padrão é 7 dias quando omitido. Isso é independente da claim `exp` de curta duração do próprio token. |
