...
description: Sirva documentação em vários idiomas com um seletor de idioma, navegação própria para cada idioma e conteúdo traduzido.
---
...
<Note>
  O Jamdesk não traduz o conteúdo por você. Você fornece os arquivos MDX traduzidos; o Jamdesk gerencia o roteamento, a navegação, o seletor de idioma e os estilos RTL. Traga as traduções do seu próprio fluxo de trabalho (tradutores humanos, tradução automática ou um LLM) e coloque-as em diretórios com prefixo de idioma.
</Note>

## Configuração

Envolva a navegação em uma matriz `languages`, com cada idioma contendo sua própria estrutura de navegação:
...
Organize o conteúdo traduzido em diretórios com prefixo de idioma:
...
Referencie as páginas na navegação usando o caminho completo (com o prefixo de idioma):
...
As strings de interface integradas (o botão **Search**, o botão **Ask AI** e o menu suspenso de abas **More**) são traduzidas automaticamente para todos os idiomas compatíveis. Não é necessário configurá-las.

## Idioma padrão

O primeiro idioma da matriz é o padrão. Os usuários que acessarem sua documentação verão primeiro esse idioma. O seletor de idioma permite que eles alterem a seleção.
...
Para páginas que não devem ser traduzidas (como a referência da API), você pode referenciar a mesma página em todos os idiomas:
...
| `description` do esquema e `description` das propriedades | ponteiros `$ref`, conteúdo das cargas `example` / `examples` |

### Comportamento de fallback

Se uma página for solicitada por uma URL localizada, mas não existir uma especificação traduzida, o Jamdesk renderizará a especificação em inglês dentro do shell da página traduzida. Os usuários verão um bloco de endpoint em vários idiomas, em vez de um erro 404. Esse comportamento corresponde ao comportamento mais amplo de traduções parciais das páginas MDX.

<Note>
  A visualização local da CLI `jamdesk` (`jamdesk dev`) resolve as especificações OpenAPI apenas pelo nome do arquivo. Ela ainda não aplica a busca pelo sufixo de idioma. Ao trabalhar localmente nas traduções, use a URL de visualização de produção do Jamdesk (`<project>.jamdesk.app/<lang>/...`) ou substitua temporariamente o arquivo de origem. Isso afeta apenas o desenvolvimento local; as renderizações hospedadas e de ISR selecionam corretamente a especificação traduzida.
</Note>

### Removendo uma especificação específica de idioma

Excluir `api.<lang>.yaml` faz com que a página use a especificação em inglês no próximo carregamento (não é necessário um novo build). Para traduzir novamente do zero, exclua o arquivo e gere-o novamente. Não é necessária nenhuma alteração em `docs.json` — a resolução da especificação é baseada exclusivamente no nome do arquivo.
...
Observe que `/tickets`, `post` e `createTicket` permanecem idênticos. Apenas a prosa é alterada.

## Fluxo de trabalho de tradução

<Steps>
  <Step title="Comece pelo inglês">
    Escreva primeiro a documentação em inglês. Ela se torna sua fonte de verdade.
  </Step>
  ...
  <Step title="Traduza o conteúdo">
    Copie os arquivos em inglês para os diretórios de idiomas e traduza-os. Mantenha os mesmos nomes de arquivo.
  </Step>
  <Step title="Atualize o docs.json">
    Adicione entradas de idioma à configuração de navegação.
  </Step>
</Steps>
