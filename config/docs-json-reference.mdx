...
The `icon` field also accepts the full `{ "name", "style", "library" }` icon object used elsewhere in `docs.json` navigation. When `popularPages` is omitted, Jamdesk shows Quick Start and Introduction by default.
