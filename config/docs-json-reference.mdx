...
**Type:** `"jam" | "nebula" | "pulsar" | "halo"` (required)
...
<Tabs>
  ...
  <Tab title="halo">
    Warm and soft with Figtree, heavily rounded surfaces, and content raised on a card.

    **Best for:** Comfortable long-form reading, approachable product docs
  </Tab>
</Tabs>
...
Control the page metadata shown on every documentation page.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `timestamp` | boolean | `false` | When `true`, shows a "Last updated on June 15, 2026"-style line in the footer of every page. The date comes from the last Git commit that changed that page, so it stays accurate automatically on each build. |

The date renders on your published site and in `jamdesk dev`. It reflects the most recent commit that touched each page's file, so pages you haven't edited keep their original date.
...
Control how visitors reach the translated version of your docs.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `autoRedirect` | boolean | `false` | When `true`, a first-time visitor arriving at your docs root is sent to the language matching their browser's `Accept-Language` header. Requires two or more entries in `navigation.languages`. |

Only roots redirect. A deep link such as `/guides/authentication` always serves the page it names, so a link you paste into a ticket opens the same page for everyone. Search engine crawlers are never redirected, so your `hreflang` tags keep deciding what gets indexed.

The visitor's language is remembered for a year. Choosing a language from the language picker overrides the automatic match from then on.

See [Languages → Automatic language routing](/setup/languages#automatic-language-routing) for the full behavior.

## Banner

### banner

Show a site-wide announcement bar pinned to the top of every page, above the header, at full width, in your theme's accent color. Use it for launches, migrations, maintenance windows, or any message every visitor should see.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | string | - | **Required.** The banner text. Supports basic inline formatting: links `[text](url)`, **bold** (`**text**`), and *italic* (`*text*`). Custom MDX components are not supported. |
| `dismissible` | boolean | `false` | When `true`, shows a close button. After a visitor dismisses the banner it stays hidden for them until you change `content`. Editing the message brings it back. |

The banner renders on your published site and in `jamdesk dev`. It is configured globally (one banner for the whole site); per-tab and per-language banners are not currently supported.
...
<Warning>
An `asyncapi` key is accepted anywhere `openapi` is, but Jamdesk does not render AsyncAPI specs yet — nothing is generated from it. `jamdesk validate` warns when it finds one, so a config that looks supported doesn't stay that way until you build the site.
</Warning>

### navigation openapi (generated pages)

**Type:** `object`

Put an `openapi` object on a navigation **tab** and set `generate: true`, and Jamdesk builds one endpoint page per operation in that spec at build time — plus the sidebar groups to hold them. Nothing to author, nothing to commit.
...
|-----|------|-------------|
| `source` | string | Path to the spec, relative to your `docs.json` |
| `generate` | boolean | `true` builds the pages and the sidebar. Without it the key stays inert |

Generated pages live under the tab's own name, slugified, with a slug built from the method and path — the tab above puts `POST /tickets` at `/api-reference/post-tickets`. Operations are grouped by their first `tag`; specs with no tags fall back to the first meaningful path segment, so `/api/v1/auctions/{auctionId}` lands under **Auctions**. Titles come from the operation `summary` when the spec has one, otherwise from the method and path — and an operation that addresses a single resource is titled in the singular (`GET /users/{id}` → "Get User").

<Note>
`generate` is opt-in by construction. A bare `"openapi": "/openapi/api.yaml"` on a tab, or an object without `generate: true`, keeps doing exactly what it did before — so no existing config gains a hundred pages on its next build.
</Note>

A committed `.mdx` file always wins a slug collision, so you can adopt generation gradually: turn it on, then delete your hand-written endpoint pages one at a time as you're ready.

If you later rename a path in your spec, the generated slug moves with it. Jamdesk keeps a per-operation slug history and emits a redirect from every former URL to the current one on each build, so inbound links and bookmarks survive the rename. Your own [redirects](/setup/redirects) and any live page still take precedence.

**Current limits.** Generation runs on top-level `navigation.tabs` only — not on groups, anchors, or tabs nested under `languages` or `versions` — and produces pages for your default language only. A `directory` key is accepted by the schema but has no effect on where pages land.

### api.mdx.server

**Type:** `string`

Base URL used in code examples on `api:` frontmatter pages (the MDX-authored kind, not `openapi:` pages, which take their servers from the spec).
...
<Note>
An array is accepted for compatibility, but only the first entry is ever used — everything after it is discarded. `jamdesk validate` warns when you list more than one.
</Note>
...
Customize the documentation search bar. Search works out of the box; you only need this field to change the placeholder text or surface popular pages in the empty state.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `prompt` | string | `Search documentation…` | Placeholder text shown in the search input |
| `popularPages` | array | Quick Start, Introduction | Quick-access links shown before the visitor types a query |
...
Each entry in `popularPages` accepts:

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `title` | string | Yes | Label shown for the link |
| `slug` | string | Yes | Page path, without a leading slash or `.mdx` extension (for example `quickstart`, or `guides/authentication` for the file `guides/authentication.mdx`) |
| `icon` | string | No | Font Awesome icon name shown beside the link (for example `rocket` or `bell`) |

The `icon` field also accepts the full `{ "name", "style", "library" }` object. See [Icon object form](/content/icons#icon-object-form). When `popularPages` is omitted, Jamdesk shows Quick Start and Introduction by default.
...
Gate the whole site behind your own login system. Your backend signs a short-lived token for each signed-in user and Jamdesk exchanges it for a session cookie. The signing key is generated in the dashboard, not in this file. `auth.jwt` and `auth.password` can't both be enabled; a build with both on fails with a `config_error`.
...
| Field | Type | Description |
|-------|------|-------------|
| `enabled` | `boolean` | Turn JWT authentication on. Every page needs a session except anything marked public. |
| `loginUrl` | `string` | Required when `enabled` is `true`. An absolute `https://` URL on your side. Visitors without a session are sent here with `?redirect=<path>` so you can bring them back to the page they asked for. |
| `public` | `string[]` | Path globs that stay reachable without signing in. Same syntax as `auth.password.public`, and merged with frontmatter `public: true` and `"public": true` on navigation groups. |

Per-page access on top of this comes from `groups` in page frontmatter. See [JWT Authentication](/setup/jwt-authentication) for the token format, the redirect flow, and how groups work.
