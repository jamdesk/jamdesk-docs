...
<Note>`bash` is an alias for `curl` — they produce the same output. Use whichever label you prefer.</Note>
...
Enable automatic WebP conversion for PNG and JPG assets during builds. Converted files are usually 60-80% smaller than the originals with no visible quality loss, and all references in your MDX, custom CSS, custom JS, and `docs.json` are rewritten to point at the new files. Favicons, `og:image`, and `twitter:image` stay in their original format because not every social crawler or email client renders WebP reliably.
