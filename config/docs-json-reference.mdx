...
Enable the interactive [API Playground](/api-reference/playground) on endpoint pages. When enabled, a "Try it" button appears on every `openapi:` and `api:` page.

| Value | Behavior |
|-------|----------|
| `"interactive"` | Full playground — fill params, generate code, send requests |
| `"simple"` | Fill params and copy code, but no Send button |
| `"none"` | Playground disabled (default) |
